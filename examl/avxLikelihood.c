...
} absMask_AVX = { { 0x7fffffffffffffffULL, 0x7fffffffffffffffULL,
                    0x7fffffffffffffffULL, 0x7fffffffffffffffULL } };

static inline __m256d
hadd4(__m256d v, __m256d u)
{
  __m256d a, b;
...
void
newviewGTRGAMMA_AVX(int tipCase, double* x1, double* x2, double* x3,
                    double* extEV, double* tipVector, unsigned char* tipX1,
                    unsigned char* tipX2, const int n, double* left,
                    double* right, int* wgt, int* scalerIncrement)
{
  int i, k, scale, addScale = 0;
  ...
  switch (tipCase) {
    case TIP_TIP: {
      double *uX1, umpX1[1024] __attribute__((aligned(BYTE_ALIGNMENT))), *uX2,
        umpX2[1024] __attribute__((aligned(BYTE_ALIGNMENT)));

      for (i = 1; i < 16; i++) {
        __m256d tv = _mm256_load_pd(&(tipVector[i * 4]));

        int j;

        for (j = 0; j < 4; j++)
          for (k = 0; k < 4; k++) {
            __m256d left1 = _mm256_load_pd(&left[j * 16 + k * 4]);
        ...
        for (j = 0; j < 4; j++)
          for (k = 0; k < 4; k++) {
            __m256d left1 = _mm256_load_pd(&right[j * 16 + k * 4]);
      ...
      for (i = 0; i < n; i++) {
        ...
        for (k = 0; k < 4; k++) {
          __m256d xv = _mm256_setzero_pd();

          int l;

          for (l = 0; l < 4; l++) {
            __m256d x1v = _mm256_mul_pd(_mm256_load_pd(&uX1[k * 16 + l * 4]),
                                        _mm256_load_pd(&uX2[k * 16 + l * 4]));

            __m256d evv = _mm256_load_pd(&extEV[l * 4]);
#ifdef _FMA
    ...
    case TIP_INNER: {
      double *uX1, umpX1[1024] __attribute__((aligned(BYTE_ALIGNMENT)));
      ...
      for (i = 0; i < n; i++) {
        ...
        for (k = 0; k < 4; k++) {
          __m256d xvr = _mm256_load_pd(&(x2[i * 16 + k * 4]));
          ...
          for (l = 0; l < 4; l++) {
            __m256d x1v = _mm256_load_pd(&uX1[k * 16 + l * 4]),
                    x2v = _mm256_mul_pd(xvr,
                                        _mm256_load_pd(&right[k * 16 + l * 4]));
          ...
          if (scale) {
            __m256d v1 = _mm256_and_pd(xv[k], absMask_AVX.m);
    ...
    case INNER_INNER: {
      for (i = 0; i < n; i++) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          for (l = 0; l < 4; l++) {
            __m256d x1v =
                      _mm256_mul_pd(xvl, _mm256_load_pd(&left[k * 16 + l * 4])),
                    x2v = _mm256_mul_pd(xvr,
                                        _mm256_load_pd(&right[k * 16 + l * 4]));
...
void
newviewGTRCAT_AVX(int tipCase, double* EV, int* cptr, double* x1_start,
  ...
  switch (tipCase) {
    case TIP_TIP:
      for (i = 0; i < n; i++) {
        ...
        __m256d vv = _mm256_setzero_pd();

        for (l = 0; l < 4; l++) {
          __m256d x1v = _mm256_mul_pd(_mm256_load_pd(x1),
                                      _mm256_load_pd(&le[l * 4])),
                  x2v = _mm256_mul_pd(_mm256_load_pd(x2),
                                      _mm256_load_pd(&ri[l * 4]));

          x1v = hadd4(x1v, x2v);

          __m256d evv = _mm256_load_pd(&EV[l * 4]);
#ifdef _FMA
    ...
    case TIP_INNER:
      for (i = 0; i < n; i++) {
        ...
        __m256d v1 = _mm256_and_pd(vv, absMask_AVX.m);
...
void
newviewGTRCATPROT_AVX(int tipCase, double* extEV, int* cptr, double* x1,
  ...
  switch (tipCase) {
    case TIP_TIP: {
      for (i = 0; i < n; i++) {
        ...
        for (l = 0; l < 20; l++) {
          __m256d x1v = _mm256_setzero_pd(), x2v = _mm256_setzero_pd();
...
#else
          x1v = _mm256_add_pd(
            x1v, _mm256_mul_pd(_mm256_load_pd(&vl[0]), _mm256_load_pd(&lv[0])));
          x1v = _mm256_add_pd(
            x1v, _mm256_mul_pd(_mm256_load_pd(&vl[4]), _mm256_load_pd(&lv[4])));
          x1v = _mm256_add_pd(
            x1v, _mm256_mul_pd(_mm256_load_pd(&vl[8]), _mm256_load_pd(&lv[8])));
          x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[12]),
                                                 _mm256_load_pd(&lv[12])));
          x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[16]),
                                                 _mm256_load_pd(&lv[16])));

          x2v = _mm256_add_pd(
            x2v, _mm256_mul_pd(_mm256_load_pd(&vr[0]), _mm256_load_pd(&rv[0])));
          x2v = _mm256_add_pd(
            x2v, _mm256_mul_pd(_mm256_load_pd(&vr[4]), _mm256_load_pd(&rv[4])));
          x2v = _mm256_add_pd(
            x2v, _mm256_mul_pd(_mm256_load_pd(&vr[8]), _mm256_load_pd(&rv[8])));
          x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[12]),
                                                 _mm256_load_pd(&rv[12])));
          x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[16]),
                                                 _mm256_load_pd(&rv[16])));
#endif
    ...
    case TIP_INNER:
      for (i = 0; i < n; i++) {
        ...
        for (l = 0; scale && (l < 20); l += 4) {
          __m256d v1 = _mm256_and_pd(vv[l / 4], absMask_AVX.m);
          v1 = _mm256_cmp_pd(v1, minlikelihood_avx, _CMP_LT_OS);
        ...
        if (scale) {
          __m256d twoto = _mm256_set1_pd(twotothe256);
...
void
newviewGTRGAMMAPROT_AVX_LG4(int tipCase, double* x1, double* x2, double* x3,
                            double* extEV[4], double* tipVector[4], int* ex3,
                            unsigned char* tipX1, unsigned char* tipX2, int n,
                            double* left, double* right, int* wgt,
                            int* scalerIncrement, const boolean useFastScaling)
{
  double *uX1, *uX2, *v, x1px2, *vl, *vr;
...
#define GCC_VERSION                                                            \
  ...
  __m256d bitmask = _mm256_set_pd(0, 0, 0, -1);
#else
  __m256i bitmask = _mm256_set_epi32(0, 0, 0, 0, 0, 0, -1, -1);
#endif

  switch (tipCase) {
    case TIP_TIP: {
      double umpX1[1840] __attribute__((aligned(BYTE_ALIGNMENT))),
        umpX2[1840] __attribute__((aligned(BYTE_ALIGNMENT)));

      for (i = 0; i < 23; i++) {
        for (k = 0; k < 80; k++) {
          double *ll = &left[k * 20], *rr = &right[k * 20];

          __m256d umpX1v = _mm256_setzero_pd(), umpX2v = _mm256_setzero_pd();
...
#else
            umpX1v =
              _mm256_add_pd(umpX1v, _mm256_mul_pd(vv, _mm256_load_pd(&ll[l])));
            umpX2v =
              _mm256_add_pd(umpX2v, _mm256_mul_pd(vv, _mm256_load_pd(&rr[l])));
#endif
          }
      ...
      for (i = 0; i < n; i++) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          for (l = 0; l < 20; l++) {
            ...
            vv[0] = _mm256_add_pd(
              vv[0],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[k][l * 20 + 0])));
#endif
            _mm256_store_pd(&v[0], vv[0]);
            ...
            vv[1] = _mm256_add_pd(
              vv[1],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[k][l * 20 + 4])));
#endif
            _mm256_store_pd(&v[4], vv[1]);
            ...
            vv[2] = _mm256_add_pd(
              vv[2],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[k][l * 20 + 8])));
#endif
            _mm256_store_pd(&v[8], vv[2]);
            ...
            vv[3] = _mm256_add_pd(
              vv[3],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[k][l * 20 + 12])));
#endif
            _mm256_store_pd(&v[12], vv[3]);
            ...
            vv[4] = _mm256_add_pd(
              vv[4],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[k][l * 20 + 16])));
#endif
            _mm256_store_pd(&v[16], vv[4]);
          }
        }
        ...
        if (scale) {
          __m256d twotothe256v =
            _mm256_set_pd(twotothe256, twotothe256, twotothe256, twotothe256);
          for (l = 0; l < 80; l += 4) {
            __m256d vv = _mm256_load_pd(&v[l]);
...
#else
            vv[0] = _mm256_add_pd(
              vv[0], _mm256_mul_pd(al, _mm256_load_pd(&extEV[k][20 * l + 0])));
#endif
            _mm256_store_pd(&v[0], vv[0]);
...
#else
            vv[1] = _mm256_add_pd(
              vv[1], _mm256_mul_pd(al, _mm256_load_pd(&extEV[k][20 * l + 4])));
#endif
            _mm256_store_pd(&v[4], vv[1]);
...
#else
            vv[2] = _mm256_add_pd(
              vv[2], _mm256_mul_pd(al, _mm256_load_pd(&extEV[k][20 * l + 8])));
#endif
            _mm256_store_pd(&v[8], vv[2]);
...
#else
            vv[3] = _mm256_add_pd(
              vv[3], _mm256_mul_pd(al, _mm256_load_pd(&extEV[k][20 * l + 12])));
#endif
            _mm256_store_pd(&v[12], vv[3]);
...
#else
            vv[4] = _mm256_add_pd(
              vv[4], _mm256_mul_pd(al, _mm256_load_pd(&extEV[k][20 * l + 16])));
#endif
            _mm256_store_pd(&v[16], vv[4]);
          }
        }
...
void
newviewGTRGAMMAPROT_AVX(int tipCase, double* x1, double* x2, double* x3,
                        double* extEV, double* tipVector, unsigned char* tipX1,
      ...
      for (i = 0; i < n; i++) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          for (l = 0; l < 20; l++) {
...
#else
            vv[0] = _mm256_add_pd(
              vv[0], _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 0])));
#endif
            _mm256_store_pd(&v[0], vv[0]);
...
#else
            vv[1] = _mm256_add_pd(
              vv[1], _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 4])));
#endif
            _mm256_store_pd(&v[4], vv[1]);
...
#else
            vv[2] = _mm256_add_pd(
              vv[2], _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 8])));
#endif
            _mm256_store_pd(&v[8], vv[2]);
            ...
            vv[3] = _mm256_add_pd(
              vv[3],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 12])));
#endif
            _mm256_store_pd(&v[12], vv[3]);
            ...
            vv[4] = _mm256_add_pd(
              vv[4],
              _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 16])));
#endif
            _mm256_store_pd(&v[16], vv[4]);
          }
        }
...
#else
            vv[0] = _mm256_add_pd(
              vv[0], _mm256_mul_pd(al, _mm256_load_pd(&extEV[20 * l + 0])));
#endif
            _mm256_store_pd(&v[0], vv[0]);
...
#else
            vv[1] = _mm256_add_pd(
              vv[1], _mm256_mul_pd(al, _mm256_load_pd(&extEV[20 * l + 4])));
#endif
            _mm256_store_pd(&v[4], vv[1]);
...
#else
            vv[2] = _mm256_add_pd(
              vv[2], _mm256_mul_pd(al, _mm256_load_pd(&extEV[20 * l + 8])));
#endif
            _mm256_store_pd(&v[8], vv[2]);
...
#else
            vv[3] = _mm256_add_pd(
              vv[3], _mm256_mul_pd(al, _mm256_load_pd(&extEV[20 * l + 12])));
#endif
            _mm256_store_pd(&v[12], vv[3]);
...
#else
            vv[4] = _mm256_add_pd(
              vv[4], _mm256_mul_pd(al, _mm256_load_pd(&extEV[20 * l + 16])));
#endif
            _mm256_store_pd(&v[16], vv[4]);
          }
        }
...
newviewGTRGAMMA_AVX_GAPPED_SAVE(
  int tipCase, double* x1_start, double* x2_start, double* x3_start,
  double* extEV, double* tipVector, int* ex3, unsigned char* tipX1,
  unsigned char* tipX2, const int n, double* left, double* right, int* wgt,
  int* scalerIncrement, const boolean useFastScaling, unsigned int* x1_gap,
  unsigned int* x2_gap, unsigned int* x3_gap, double* x1_gapColumn,
  double* x2_gapColumn, double* x3_gapColumn)
{
  int i, k, scale, scaleGap, addScale = 0;

  __m256d minlikelihood_avx = _mm256_set1_pd(minlikelihood),
          twoto = _mm256_set1_pd(twotothe256);

  double *x1, *x2, *x3, *x1_ptr = x1_start, *x2_ptr = x2_start;
  ...
  switch (tipCase) {
    ...
    case TIP_INNER: {
...

      {
        ...
        for (k = 0; k < 4; k++) {
          __m256d xvr = _mm256_load_pd(&(x2[k * 4]));
        ...
        if ((x3_gap[i / 32] & mask32[i % 32])) {
          ...
          for (k = 0; k < 4; k++) {
            ...
            for (l = 0; l < 4; l++) {
              __m256d x1v = _mm256_load_pd(&uX1[k * 16 + l * 4]),
                      x2v = _mm256_mul_pd(
                        xvr, _mm256_load_pd(&right[k * 16 + l * 4]));
      ...
      for (i = 0; i < n; i++) {
        if (x3_gap[i / 32] & mask32[i % 32]) {
          ...
          for (k = 0; k < 4; k++) {
            ...
            for (l = 0; l < 4; l++) {
              __m256d x1v = _mm256_mul_pd(
                        xvl, _mm256_load_pd(&left[k * 16 + l * 4])),
                      x2v = _mm256_mul_pd(
                        xvr, _mm256_load_pd(&right[k * 16 + l * 4]));
...
newviewGTRCAT_AVX_GAPPED_SAVE(
  int tipCase, double* EV, int* cptr, double* x1_start, double* x2_start,
  double* x3_start, double* tipVector, int* ex3, unsigned char* tipX1,
  unsigned char* tipX2, int n, double* left, double* right, int* wgt,
  int* scalerIncrement, const boolean useFastScaling, unsigned int* x1_gap,
  unsigned int* x2_gap, unsigned int* x3_gap, double* x1_gapColumn,
  double* x2_gapColumn, double* x3_gapColumn, const int maxCats)
{
  double *le, *ri, *x1, *x2, *x3, *x1_ptr = x1_start, *x2_ptr = x2_start,
                                  *x3_ptr = x3_start;

  int i, scaleGap = 0, addScale = 0;
...

  {
    ...
    for (l = 0; l < 4; l++) {
      __m256d x1v =
                _mm256_mul_pd(_mm256_load_pd(x1), _mm256_load_pd(&le[l * 4])),
              x2v =
                _mm256_mul_pd(_mm256_load_pd(x2), _mm256_load_pd(&ri[l * 4]));
...
newviewGTRCATPROT_AVX_GAPPED_SAVE(
  int tipCase, double* extEV, int* cptr, double* x1, double* x2, double* x3,
  double* tipVector, int* ex3, unsigned char* tipX1, unsigned char* tipX2,
  int n, double* left, double* right, int* wgt, int* scalerIncrement,
  const boolean useFastScaling, unsigned int* x1_gap, unsigned int* x2_gap,
  unsigned int* x3_gap, double* x1_gapColumn, double* x2_gapColumn,
  double* x3_gapColumn, const int maxCats)
{
  double *le, *ri, *v, *vl, *vr, *x1_ptr = x1, *x2_ptr = x2, *x3_ptr = x3;

  int i, l, scale, addScale = 0, scaleGap = 0;
...

  {
    ...
    for (l = 0; l < 20; l++) {
      ...
      x1v = _mm256_add_pd(
        x1v, _mm256_mul_pd(_mm256_load_pd(&vl[8]), _mm256_load_pd(&lv[8])));
      x1v = _mm256_add_pd(
        x1v, _mm256_mul_pd(_mm256_load_pd(&vl[12]), _mm256_load_pd(&lv[12])));
      x1v = _mm256_add_pd(
        x1v, _mm256_mul_pd(_mm256_load_pd(&vl[16]), _mm256_load_pd(&lv[16])));
      ...
      x2v = _mm256_add_pd(
        x2v, _mm256_mul_pd(_mm256_load_pd(&vr[8]), _mm256_load_pd(&rv[8])));
      x2v = _mm256_add_pd(
        x2v, _mm256_mul_pd(_mm256_load_pd(&vr[12]), _mm256_load_pd(&rv[12])));
      x2v = _mm256_add_pd(
        x2v, _mm256_mul_pd(_mm256_load_pd(&vr[16]), _mm256_load_pd(&rv[16])));
      ...
      for (i = 0; i < n; i++) {
        if (noGap(x3_gap, i)) {
          ...
          for (l = 0; l < 20; l++) {
...
#else
            x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[0]),
                                                   _mm256_load_pd(&lv[0])));
            x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[4]),
                                                   _mm256_load_pd(&lv[4])));
            x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[8]),
                                                   _mm256_load_pd(&lv[8])));
            x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[12]),
                                                   _mm256_load_pd(&lv[12])));
            x1v = _mm256_add_pd(x1v, _mm256_mul_pd(_mm256_load_pd(&vl[16]),
            ...
            x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[0]),
                                                   _mm256_load_pd(&rv[0])));
            x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[4]),
                                                   _mm256_load_pd(&rv[4])));
            x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[8]),
                                                   _mm256_load_pd(&rv[8])));
            x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[12]),
                                                   _mm256_load_pd(&rv[12])));
            x2v = _mm256_add_pd(x2v, _mm256_mul_pd(_mm256_load_pd(&vr[16]),
...
newviewGTRGAMMAPROT_AVX_GAPPED_SAVE(
  ...
  double* x2_gapColumn, double* x3_gapColumn)
{
  double *x1 = x1_start, *x2 = x2_start, *x3_ptr = x3_start, *x2_ptr = x2_start,
         *x1_ptr = x1_start, *uX1, *uX2, *v, x1px2, *vl, *vr;

  int i, j, l, k, gapScaling = 0, scale, addScale = 0;
        ...
        if ((x3_gap[i / 32] & mask32[i % 32])) {
          ...
          for (k = 0; k < 4; k++) {
            ...
            for (l = 0; l < 20; l++) {
              ...
              vv[0] = _mm256_add_pd(
                vv[0],
                _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 0])));
#endif
              _mm256_store_pd(&v[0], vv[0]);
              ...
              vv[1] = _mm256_add_pd(
                vv[1],
                _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 4])));
#endif
              _mm256_store_pd(&v[4], vv[1]);
              ...
              vv[2] = _mm256_add_pd(
                vv[2],
                _mm256_mul_pd(x1px2v, _mm256_load_pd(&extEV[l * 20 + 8])));
#endif
              _mm256_store_pd(&v[8], vv[2]);
