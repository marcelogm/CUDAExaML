...
stringHashtable*
initStringHashTable(hashNumberType n)
{
  /*
   init with primes
*/

  static const hashNumberType initTable[] = {
...
static const  hashNumberType initTable[] = {64, 128, 256, 512, 1024, 2048, 4096,
8192, 16384,
...
static hashNumberType
hashString(char* p, hashNumberType tableSize)
{
  hashNumberType h = 0;
...
void
addword(char* s, stringHashtable* h, int nodeNumber)
{
...
int
lookupWord(char* s, stringHashtable* h)
{
  hashNumberType position = hashString(s, h->tableSize);
  stringEntry* p = h->table[position];
...
static double
getBranchLength(tree* tr, int perGene, nodeptr p)
{
  double z = 0.0, x = 0.0;
...
static char*
Tree2StringREC(char* treestr, tree* tr, nodeptr p, boolean printBranchLengths,
               boolean printNames, boolean printLikelihood, boolean rellTree,
               boolean finalPrint, int perGene, boolean branchLabelSupport,
               boolean printSHSupport)
{
  char* nameptr;
  ...
  if (isTip(p->number, tr->mxtips)) {
    ...
    *treestr++ = '(';
    treestr = Tree2StringREC(treestr, tr, p->next->back, printBranchLengths,
                             printNames, printLikelihood, rellTree, finalPrint,
                             perGene, branchLabelSupport, printSHSupport);
    *treestr++ = ',';
    treestr =
      Tree2StringREC(treestr, tr, p->next->next->back, printBranchLengths,
                     printNames, printLikelihood, rellTree, finalPrint, perGene,
                     branchLabelSupport, printSHSupport);
    if (p == tr->start->back) {
      *treestr++ = ',';
      treestr = Tree2StringREC(
        treestr, tr, p->back, printBranchLengths, printNames, printLikelihood,
        rellTree, finalPrint, perGene, branchLabelSupport, printSHSupport);
    }
    *treestr++ = ')';
  }
  ...
  if (p == tr->start->back) {
    ...
    if (rellTree || branchLabelSupport || printSHSupport) {
      ...
      if(printSHSupport)
        sprintf(treestr, ":%8.20f[%d]", getBranchLength(tr, perGene, p),
      p->bInf->support);
      */
...
char*
Tree2String(char* treestr, tree* tr, nodeptr p, boolean printBranchLengths,
            boolean printNames, boolean printLikelihood, boolean rellTree,
            boolean finalPrint, int perGene, boolean branchLabelSupport,
            boolean printSHSupport)
{
  ...
  Tree2StringREC(treestr, tr, p, printBranchLengths, printNames,
                 printLikelihood, rellTree, finalPrint, perGene,
                 branchLabelSupport, printSHSupport);
...
void
printTreePerGene(tree* tr, analdef* adef, char* fileName, char* permission)
{
  FILE* treeFile;
...
static int
treeFinishCom(FILE* fp, char** strp)
{
  int ch;

  while ((ch = getc(fp)) != EOF && ch != ']') {
    if (strp != NULL)
      *(*strp)++ = ch; /* save character  */
    if (ch == '[') {   /* nested comment; find its end */
      if ((ch = treeFinishCom(fp, strp)) == EOF)
        break;
      if (strp != NULL)
        *(*strp)++ = ch; /* save closing ]  */
    }
  }

  if (strp != NULL)
    **strp = '\0'; /* terminate string  */
  return ch;
} /* treeFinishCom */
...
{                              /* treeGetCh */
  ...
  while ((ch = getc(fp)) != EOF) {
    if (whitechar(ch))
      ...
      if ((ch = treeFinishCom(fp, (char**)NULL)) == EOF)
        break;
    } else
      break;
  }
...
static boolean
treeGetLabel(FILE* fp, char* lblPtr, int maxlen)
{
  int ch;
...
static boolean
treeFlushLabel(FILE* fp)
{
  return treeGetLabel(fp, (char*)NULL, (int)0);
}

static int
treeFindTipByLabelString(char* str, tree* tr, boolean check)
{
...
int
treeFindTipName(FILE* fp, tree* tr, boolean check)
{
  char str[nmlngth + 2];
  int n;
...
static void
treeEchoContext(FILE* fp1, FILE* fp2, int n)
{ /* treeEchoContext */
  int ch;
  boolean waswhite;
...
static boolean
treeProcessLength(FILE* fp, double* dptr)
{
  int ch;
...
static boolean
treeNeedCh(FILE* fp, int c1, char* where)
{
  int c2;
...
static boolean
addElementLen(FILE* fp, tree* tr, nodeptr p, boolean readBranchLengths,
              boolean readNodeLabels, int* lcount)
{
  nodeptr q;
  int n, ch, fres;
  ...
  if ((ch = treeGetCh(fp)) == '(') {
    ...
    if (!addElementLen(fp, tr, q->next, readBranchLengths, readNodeLabels,
                       lcount))
      return FALSE;
    if (!treeNeedCh(fp, ',', "in"))
      return FALSE;
    if (!addElementLen(fp, tr, q->next->next, readBranchLengths, readNodeLabels,
                       lcount))
      return FALSE;
    if (!treeNeedCh(fp, ')', "in"))
      return FALSE;
    ...
    ungetc(ch, fp);
    if ((n = treeFindTipName(fp, tr, TRUE)) <= 0)
      return FALSE;
    q = tr->nodep[n];
  ...
  if (readBranchLengths) {
    ...
    if (!treeNeedCh(fp, ':', "in"))
      return FALSE;
    if (!treeProcessLength(fp, &branch))
      return FALSE;
...
static nodeptr
uprootTree(tree* tr, nodeptr p, boolean readBranchLengths)
{
  nodeptr q, r, s, start;
  int n, i;
...
int
treeReadLen(FILE* fp, tree* tr, boolean readBranches, boolean readNodeLabels,
            boolean topologyOnly)
{
  ...
  if (!tr->rooted) {
    if ((ch = treeGetCh(fp)) == ',') {
      if (!addElementLen(fp, tr, p->next->next, readBranches, readNodeLabels,
                         &lcount))
        assert(0);
...
static boolean
addElementLenMULT(FILE* fp, tree* tr, nodeptr p, int partitionCounter,
                  int* partCount)
{
  nodeptr q, r, s;
  int n, ch, fres, rn;
  ...
  if ((ch = treeGetCh(fp)) == '(') {
    *partCount = *partCount + 1;
    ...
    if (!addElementLenMULT(fp, tr, q->next, old, partCount))
      return FALSE;
    if (!treeNeedCh(fp, ',', "in"))
      return FALSE;
    if (!addElementLenMULT(fp, tr, q->next->next, old, partCount))
      return FALSE;
...
boolean
treeReadLenMULT(FILE* fp, tree* tr, int* partCount)
{
  nodeptr p, r, s;
  int i, ch, n, rn;
  ...
  if (!addElementLenMULT(fp, tr, p, partitionCounter, partCount))
    return FALSE;
  if (!treeNeedCh(fp, ',', "in"))
    return FALSE;
  if (!addElementLenMULT(fp, tr, p->next, partitionCounter, partCount))
    return FALSE;
  if (!tr->rooted) {
    if ((ch = treeGetCh(fp)) == ',') {
      if (!addElementLenMULT(fp, tr, p->next->next, partitionCounter,
                             partCount))
        return FALSE;
