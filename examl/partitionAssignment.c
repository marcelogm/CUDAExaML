...
void
initializePartitionAssignment(PartitionAssignment** pAssPtr, pInfo** partitions,
                              int numPart, int numProc)
{
  int i;
  ...
  *pAssPtr = (PartitionAssignment*)calloc(1, sizeof(PartitionAssignment));
  ...
  pAss->partitions =
    (Partition*)calloc((size_t)pAss->numPartitions, sizeof(Partition));
  ...
  pAss->assignPerProc =
    (Assignment**)calloc((size_t)pAss->numProc, sizeof(Assignment*));
  pAss->numAssignPerProc = (int*)calloc((size_t)pAss->numProc, sizeof(int));
}

void
deletePartitionAssignment(PartitionAssignment* pAss)
{
  int i;
...
static int
partSort(const void* a, const void* b)
{
  return ((const Partition*)a)->width - ((const Partition*)b)->width;
}

/**
    helper function that executes the assignment of a partial assignment (only
   numElem character are assigned)
 */
static void
assignPartitionPartial(PartitionAssignment* pa, Partition* p, int procId,
                       int* numAssigned, size_t* sizeAssigned, size_t offset,
                       size_t numElem)
{
  Assignment* a;
  ...
  pa->assignPerProc[procId] = (Assignment*)realloc(
    pa->assignPerProc[procId], newArrayLen * sizeof(Assignment));
 ...
 */
static void
assignPartitionFull(PartitionAssignment* pa, Partition* p, int procId,
                    int* numAssigned, size_t* sizeAssigned)
{
 ...
 */
static int
popAndYield(int** procsHighIter, int* procsHighStart, int** procsLowIter,
            int* procsLowStart, boolean wantLow)
{
  boolean fromHigh = FALSE, fromLow = FALSE;
  ...
  if (wantLow) {
    if (*procsLowIter - procsLowStart > 0)
      fromLow = TRUE;
    else if (*procsHighIter - procsHighStart > 0)
      fromHigh = TRUE;
  } else {
    if (*procsHighIter - procsHighStart > 0)
      fromHigh = TRUE;
    else if (*procsLowIter - procsLowStart > 0)
      fromLow = TRUE;
  }
...
static void
assignThesePartitions(PartitionAssignment* pa, Partition* partitions,
                      int numCur)
{
  int proc, remainder, /* number of processes that receive 1 character less than
                          other s */
    i, numFull = 0,    /* number of processes that cannot take any more   */
    numLow = 0,        /*  */
      *numAssigned =
        (int *)NULL, /* number of characters assigned to a process */
        *procsHighIter =
          (int *)NULL, /* stack of processes that have one assignment more
                          than others  */
          *procsLowIter = (int *)NULL,       /* stack of processes that have one
                                                assignment less than others  */
            *procsLowStart = (int *)NULL,    /* start of stack */
              *procsHighStart = (int *)NULL, /* start of stack */
    highProc, /* id of a process that potentially has more partitions assigned
                 than others   */
    lowProc;  /* id of a process that potentially has less partitions assigned
                 than others   */

  size_t totalElems = 0, *sizeAssigned = (size_t *)NULL, toAdd,
  ...
  Partition *partIter = partitions, *partEnd = partitions + numCur;
...
*/
  ...
  assert((procsHighIter - procsHighStart) + (procsLowIter - procsLowStart) +
           numFull ==
         pa->numProc);

  toAdd = (partIter < partEnd) ? partIter->width : 0;
  highProc = popAndYield(&procsHighIter, procsHighStart, &procsLowIter,
                         procsLowStart, FALSE);
  lowProc = popAndYield(&procsHighIter, procsHighStart, &procsLowIter,
                        procsLowStart, TRUE);
 ...
 */
  while (!(highProc == -1 && lowProc == -1 &&
           (procsHighIter - procsHighStart <= 0) &&
           (procsLowIter - procsLowStart <= 0))) {
    /* try to finish a assignments for a process that has many partitions  */
    if (highProc != -1 && sizeAssigned[highProc] + toAdd >= cap) {
      size_t toTransfer = cap - sizeAssigned[highProc],
             offset = partIter->width - toAdd;

      assignPartitionPartial(pa, partIter, highProc, numAssigned, sizeAssigned,
                             offset, toTransfer);
      ...
      if (sizeAssigned[lowProc] + toAdd < cap) {
        size_t offset = partIter->width - toAdd;

        assignPartitionPartial(pa, partIter, lowProc, numAssigned, sizeAssigned,
                               offset, toAdd);
        ...
        size_t toTransfer = cap - sizeAssigned[lowProc],
               offset = partIter->width - toAdd;
 ...
 */
void
assign(PartitionAssignment* pa)
{
  int partitionsHandled = 0, curType = -1, j, i;
 ...
 */
  ...
  for (j = 0; j < 3; ++j) {
    ...
    Partition* curPartitions = (Partition*)NULL;
...
void
printAssignment(Assignment a, int procid)
{
  printf("p: %d\t(%lu,%lu) -> proc %d\n", a.partId, a.offset, a.width, procid);
}

void
printAssignments(PartitionAssignment* pa)
{
  int i, j;
...
void
printLoad(PartitionAssignment* pa)
{
  int i, j, *numsPerProc = (int *)calloc((size_t)pa->numProc, sizeof(int));

  size_t* sitesPerProc = (size_t*)calloc((size_t)pa->numProc, sizeof(size_t));
 ...
 */
static void
setupBasePointersInTree(tree* tr)
{
  size_t len = 0;
  int i;
...
void
copyAssignmentInfoToTree(PartitionAssignment* pa, tree* tr)
{
  int i, numAssign = 0;
...
#ifdef _USE_OMP
void
copyThreadAssignmentInfoToTree(PartitionAssignment* pa, tree* tr)
{
  int i, j;

  /* we want to know max number of partitions assigned to a single thread ->
   * mainly for memory allocation */

  int *numsPerProc = (int *)calloc((size_t)pa->numProc, sizeof(int)),
  ...
  assert(tr->maxModelsPerThread > 0 &&
         tr->maxModelsPerThread <= pa->numPartitions);
  ...
  int threadPartSize = pa->numProc * tr->maxModelsPerThread,
      partThreadSize = pa->numPartitions * tr->maxThreadsPerModel;

  tr->threadPartAssigns =
    (Assign**)calloc((size_t)threadPartSize, sizeof(Assign*));
  tr->partThreadAssigns =
    (Assign**)calloc((size_t)partThreadSize, sizeof(Assign*));
  ...
  for (i = 0; i < pa->numProc; ++i) {
    ...
    for (j = 0; j < pa->numAssignPerProc[i]; ++j) {
      ...
      size_t ind = i * tr->maxModelsPerThread + partCount;
