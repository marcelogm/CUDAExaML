...
void
makeP_PROT_MIC(double z1, double z2, double* rptr, double* EI, double* EIGN,
               int numberOfCategories, double* left, double* right,
               boolean saveMem, int maxCat)
{
  int i, j, k, span = states * numberOfCategories;
...
void
precomputeTips_PROT_MIC(int tipCase, double* tipVector, double* left,
                        double* right, double* umpLeft, double* umpRight,
                        int numberOfCategories)
{
  ...
  const int span = states * numberOfCategories, umpSize = span * maxStateValue;
...
#pragma vector aligned
      for (int k = 0; k < span; ++k) {
        ...
        if (tipCase == TIP_TIP)
          umpRight[span * i + k] +=
            tipVector[i * states + l] * right[l * span + k];
      }
    }
  }
}
...
inline void
mic_fma4x80(const double* inv, double* outv, double* mulv)
{
  ...
  for (int l = 0; l < 80; l += 40) {
    ...
    t = _mm512_extload_pd(&inv[l], _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8,
                          _MM_HINT_NONE);
    ...
    t = _mm512_mask_extload_pd(t, k1, &inv[l], _MM_UPCONV_PD_NONE,
    ...
    t = _mm512_extload_pd(&inv[l + 20], _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8,
                          _MM_HINT_NONE);
...
void
newviewGTRGAMMAPROT_MIC(int tipCase, double* x1, double* x2, double* x3,
                        ...
                        double* umpLeft, double* umpRight)
{
  ...
  double *aEV = extEV, *aRight = right, *aLeft = left, *umpX1 = umpLeft,
         *umpX2 = umpRight;
...
#pragma unroll(10)
        for (int j = 0; j < span; j += 8) {
          ...
          //                _mm_prefetch((const char *)&x2[span*(i+1) + j],
          //                _MM_HINT_T0);
        }
...
#pragma unroll(10)
        ...
        t1 = _mm512_castsi512_pd(
          _mm512_and_epi64(_mm512_castpd_si512(t1), absMask_MIC));
        double vmax = _mm512_reduce_max_pd(t1);
        for (int l = 8; l < span; l += 8) {
          __m512d t = _mm512_load_pd(&v3[l]);
          t = _mm512_castsi512_pd(
            _mm512_and_epi64(_mm512_castpd_si512(t), absMask_MIC));
...
#pragma unroll(10)
        for (int j = 0; j < span; j += 8) {
          ...
          _mm_prefetch((const char*)&x2[span * (i + 1) + j], _MM_HINT_T1);
          //                _mm_prefetch((const char *)&x1[span*(i+1) + j],
          //                _MM_HINT_T0);
          //                _mm_prefetch((const char *)&x2[span*(i+1) + j],
          //                _MM_HINT_T0);
        }
...
double
evaluateGAMMAPROT_MIC(int* wgt, double* x1_start, double* x2_start,
...
void
sumGAMMAPROT_MIC(int tipCase, double* sumtable, double* x1_start,
...
void
coreGTRGAMMAPROT_MIC(const int upper, double* sumtable,
...
#pragma unroll(10)
      ...
      inv_1 =
        _mm512_add_pd(inv_1, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                               _mm512_castpd_si512(inv_1), _MM_PERM_BADC)));
      ...
      d1_1 = _mm512_add_pd(d1_1, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                                   _mm512_castpd_si512(d1_1), _MM_PERM_BADC)));
      ...
      d2_1 = _mm512_add_pd(d2_1, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                                   _mm512_castpd_si512(d2_1), _MM_PERM_BADC)));
 ...
 */
void
updateModel_LG4_MIC(pInfo* part)
{
  double **EV = part->EV_LG4, **tipVector = part->tipVector_LG4,
         *aEV = part->mic_EV, *aTipVector = part->mic_tipVector;

  const int states = part->states, span = 4 * states,
...
void
makeP_PROT_LG4_MIC(double z1, double z2, double* rptr, double* EI[4],
...
void
precomputeTips_PROT_LG4_MIC(int tipCase, double* tipVector[4], double* left,
                            double* right, double* umpLeft, double* umpRight,
                            int numberOfCategories)
{
  /* no precomputation needed if both children are inner nodes */
...
#pragma vector aligned
      for (int k = 0; k < span; ++k) {
        umpLeft[span * i + k] +=
          tipVector[k / 20][i * states + l] * left[l * span + k];
        if (tipCase == TIP_TIP)
          umpRight[span * i + k] +=
            tipVector[k / 20][i * states + l] * right[l * span + k];
      }
    }
  }
}
...
void
newviewGTRGAMMAPROT_LG4_MIC(int tipCase, double* x1, double* x2, double* x3,
                            double* extEV, double* tipVector,
                            unsigned char* tipX1, unsigned char* tipX2, int n,
                            double* left, double* right, int* wgt,
                            int* scalerIncrement, double* umpLeft,
                            double* umpRight)
{
  __m512d minlikelihood_MIC = _mm512_set1_pd(minlikelihood);
  __m512d twotothe256_MIC = _mm512_set1_pd(twotothe256);
...
double
evaluateGAMMAPROT_LG4_MIC(int* wgt, double* x1_start, double* x2_start,
                          double* tipVector, unsigned char* tipX1, const int n,
                          double* diagptable, double* weights)
{
  ...
  if (tipX1) {
    /* loop over the sites of this partition */
    for (int i = 0; i < n; i++) {
      const double* aTipVec = tipVector;
...
void
sumGAMMAPROT_LG4_MIC(int tipCase, double* sumtable, double* x1_start,
                     double* x2_start, double* tipVector, unsigned char* tipX1,
                     unsigned char* tipX2, int n)
{
  const double* aTipVec = tipVector;
...
void
coreGTRGAMMAPROT_LG4_MIC(const int upper, double* sumtable,
                         volatile double* ext_dlnLdlz,
                         volatile double* ext_d2lnLdlz2, double* EIGN[4],
                         double* gammaRates, double lz, int* wgt,
                         double* weights)
{
  double diagptable0[span] __attribute__((aligned(BYTE_ALIGNMENT)));
