/*  RAxML-VI-HPC (version 2.2) a program for sequential and parallel estimation
 * of phylogenetic trees
 *  Copyright August 2006 by Alexandros Stamatakis
 *
 *  Partially derived from
 *  fastDNAml, a program for estimation of phylogenetic trees from sequences by
 * Gary J. Olsen
 *
 *  and
 *
 ...
 *
 *  When publishing work that is based on the results from RAxML-VI-HPC please
 * cite:
 *
...
void
storeExecuteMaskInTraversalDescriptor(tree* tr)
{
  int model;
...
void
storeValuesInTraversalDescriptor(tree* tr, double* value)
{
  int model;
...
void
myBinFwrite(void* ptr, size_t size, size_t nmemb, FILE* byteFile)
{
  size_t bytes_read;
...
void
myBinFread(void* ptr, size_t size, size_t nmemb, FILE* byteFile)
{
  size_t bytes_read;
...
outOfMemory(void)
{
  printf("ExaML process %d was not able to allocate enough memory.\n",
         processID);
  printf("Please check the approximate memory consumption of your dataset "
         "using\n");
  printf("the memory calculator at "
...
static void
printBoth(FILE* f, const char* format, ...)
{
  if (processID == 0) {
    va_list args;
...
void
printBothOpen(const char* format, ...)
{
  if (processID == 0) {
...
static void
printBothOpenDifferentFile(char* fileName, const char* format, ...)
{
  if (processID == 0) {
    FILE* f = myfopen(fileName, "ab");
...
char
getInverseMeaning(int dataType, unsigned char state)
{
  assert(MIN_MODEL < dataType && dataType < MAX_MODEL);
...
size_t
discreteRateCategories(int rateHetModel)
{
  size_t result;
...
gettimeSrand(void)
{
  ...
  tp = time(NULL);
  localtm = *localtime(&tp);
  return 24 * 60 * 60 * localtm.tm_yday + 60 * 60 * localtm.tm_hour +
         60 * localtm.tm_min + localtm.tm_sec;
#else
  struct timeval ttime;
  gettimeofday(&ttime, NULL);
...
randum(long* seed)
{
  ...
  *seed = newseed2 << 24 | newseed1 << 12 | newseed0;
  res = 0.00390625 *
        (newseed2 + 0.000244140625 * (newseed1 + 0.000244140625 * newseed0));
...
FILE*
myfopen(const char* path, const char* mode)
{
  ...
  if (strcmp(mode, "r") == 0 || strcmp(mode, "rb") == 0) {
    if (fp)
      ...
      if (processID == 0)
        printf("The file %s you want to open for reading does not exist, "
               "exiting ...\n",
               path);
      errorExit(-1);
      return (FILE*)NULL;
    ...
    if (fp)
      ...
      if (processID == 0)
        printf("The file %s ExaML wants to open for writing or appending can "
               "not be opened [mode: %s], exiting ...\n",
               path, mode);
      errorExit(-1);
      return (FILE*)NULL;
    }
...
void
hookup(nodeptr p, nodeptr q, double* z, int numBranches)
{
  int i;
...
void
hookupDefault(nodeptr p, nodeptr q, int numBranches)
{
  int i;
...
KISS32(void)
{
  static unsigned int x = 123456789, y = 362436069, z = 21288629, w = 14921776,
                      c = 0;
...
setupTree(tree* tr)
{
  ...
  tr->partitionContributions =
    (double*)malloc(sizeof(double) * tr->NumberOfModels);
  ...
  tr->td[0].executeModel =
    (boolean*)malloc(sizeof(boolean) * tr->NumberOfModels);
  tr->td[0].parameterValues =
    (double*)malloc(sizeof(double) * tr->NumberOfModels);
...
static int
modelExists(char* model, tree* tr)
{
...
{
  if (processID == 0)
    printf("\n\nThis is %s version %s released by Alexandros Stamatakis, Andre "
           "J. Aberer, and Alexey Kozlov on %s.\n\n",
           programName, programVersion, programDate);
}

static void
printMinusFUsage(void)
{
  printf("\n");
  ...
  printf("               \"-f e\": compute the likelihood of a bunch of trees "
         "passed via -t\n");
  printf("                this option will do a quick and dirty optimization "
         "without re-optimizng\n");
  ...
  printf("               \"-f E\": compute the likelihood of a bunch of trees "
         "passed via -t\n");
  printf("                this option will do a thorough optimization that "
         "re-optimizes\n");
  printf("                the model parameters for each tree\n");

  printf("\n");

  printf("              \"-f o\": old and slower rapid hill-climbing without "
         "heuristic cutoff\n");
...
printREADME(void)
{
  if (processID == 0) {
    ...
    printf("Please send me all input files, the exact invocation, details of "
           "the HW and operating system,\n");
    ...
    printf("      -t userStartingTree|-R binaryCheckpointFile|-g "
           "constraintTree -p randomNumberSeed\n");
    printf("      [-a]\n");
    ...
    printf("\n");
    printf("      -a      use the median for the discrete approximation of the "
           "GAMMA model of rate heterogeneity\n");
    printf("\n");
    printf("              DEFAULT: OFF\n");
    printf("\n");
    printf("      -B      specify the number of best ML trees to save and "
           "print to file\n");
    printf("\n");
    printf("      -c      Specify number of distinct rate catgories for ExaML "
           "when modelOfEvolution\n");
    printf("              is set to GTRPSR\n");
    printf("              Individual per-site rates are categorized into "
           "numberOfCategories rate \n");
    printf("              categories to accelerate computations. \n");
    printf("\n");
    printf("              DEFAULT: 25\n");
    printf("\n");
    printf("      -D      ML search convergence criterion. This will break off "
           "ML searches if the relative \n");
    printf("              Robinson-Foulds distance between the trees obtained "
           "from two consecutive lazy SPR cycles\n");
    printf("              is smaller or equal to 1%s. Usage recommended for "
           "very large datasets in terms of taxa.\n",
           "%");
    printf("              On trees with more than 500 taxa this will yield "
           "execution time improvements of approximately 50%s\n",
           "%");
    printf("              While yielding only slightly worse trees.\n");
    printf("\n");
    printf("              DEFAULT: OFF\n");
    printf("\n");
    printf("      -e      set model optimization precision in log likelihood "
           "units for final\n");
    ...
    printf("\n");
    printf("      -g      Pass a multi-furcating constraint tree to ExaML. The "
           "tree needs to contain all taxa of the alignment!\n");
    printf("              When using this option you also need to specify a "
           "random number seed via \"-p\"\n");
    printf("\n");
    printf("      -h      Display this help message.\n");
    printf("\n");
    printf("      -i      Initial rearrangement setting for the subsequent "
           "application of topological \n");
    printf("              changes phase\n");
    printf("\n");
    printf("      -I      Set after how many quartet evaluations a new "
           "checkpoint will be printed.\n");
    printf("\n");
    printf("              DEFAULT: 1000\n");
    printf("\n");
    printf("      -m      Model of rate heterogeneity\n");
    printf("\n");
    printf("              select \"-m PSR\" for the per-site rate category "
           "model (this used to be called CAT in RAxML)\n");
    printf("              select \"-m GAMMA\" for the gamma model of rate "
           "heterogeneity with 4 discrete rates\n");
    printf("\n");
    printf("      -M      Switch on estimation of individual per-partition "
           "branch lengths. Only has effect when used in combination with "
           "\"-q\"\n");
    printf("              Branch lengths for individual partitions will be "
           "printed to separate files\n");
    printf("              A weighted average of the branch lengths is computed "
    ...
    printf("\n");
    printf("      -p      Specify a random number seed, required in "
           "conjunction with the \"-g\" option for constraint trees\n");
    printf("\n");
    printf("      -R      read in a binary checkpoint file called "
           "ExaML_binaryCheckpoint.RUN_ID_number\n");
    printf("\n");
    printf("      -r      Pass the number of quartets to randomly sub-sample "
           "from the possible number of quartets for the given taxon set.\n");
    printf("              Only works in combination with -f q !\n");
    printf("\n");
    printf("      -s      Specify the name of the BINARY alignment data file "
           "generated by the parser component\n");
    printf("\n");
    printf("      -S      turn on memory saving option for gappy multi-gene "
           "alignments. For large and gappy datasets specify -S to save "
           "memory\n");
    printf("              This will produce slightly different likelihood "
           "values, may be a bit slower but can reduce memory consumption\n");
    printf(
      "              from 70GB to 19GB on very large and gappy datasets\n");
    printf("\n");
    printf("      -t      Specify a user starting tree file name in Newick "
           "format\n");
    printf("\n");
    printf("      -v      Display version information\n");
    printf("\n");
    printf("      -w      FULL (!) path to the directory into which ExaML "
           "shall write its output files\n");
    printf("\n");
    printf("              DEFAULT: current directory\n");
    printf("\n");
    printf("      -Y      Pass a quartet grouping file name defining four "
           "groups from which to draw quartets\n");
    printf("              The file input format must contain 4 groups in the "
           "following form:\n");
    printf("              (Chicken, Human, Loach), (Cow, Carp), (Mouse, Rat, "
           "Seal), (Whale, Frog);\n");
    printf("              Only works in combination with -f q !\n");
    printf("\n");

    printf("\n");
    printf("      --auto-prot=ml|bic|aic|aicc When using automatic protein "
           "model selection you can chose the criterion for selecting these "
           "models.\n");
    printf("              RAxML will test all available prot subst. models "
           "except for LG4M, LG4X and GTR-based models, with and without "
           "empirical base frequencies.\n");
    printf("              You can chose between ML score based selection and "
...
static void
analyzeRunId(char id[128])
{
  int i = 0;

  while (id[i] != '\0') {
    if (i >= 128) {
      printf("Error: run id after \"-n\" is too long, it has %d characters "
...
static void
get_args(int argc, char* argv[], analdef* adef, tree* tr)
{
  boolean resultDirSet = FALSE;

  char resultDir[1024] = "",
       //*optarg,
    model[1024] = "", modelChar;

  double likelihoodEpsilon;

  int fOptionCount = 0, c, nameSet = 0, treeSet = 0, modelSet = 0,
      byteFileSet = 0, seedSet = 0;
  ...
  // while(!bad_opt && ((c =
  // mygetopt(argc,argv,"R:B:e:c:f:i:m:t:g:w:n:s:p:vhMSDa", &optind,
  // &optarg))!=-1))
  ...
  while (1) {
    static struct option long_options[2] = {
      { "auto-prot", required_argument, &flag, 1 }, { 0, 0, 0, 0 }
    };
    ...
    c = getopt_long(argc, argv, "R:B:Y:I:e:c:f:i:m:t:g:w:n:s:p:r:vhMSDa",
    ...
    if (flag > 0) {
      switch (option_index) {
        case 0: {
          char* autoModels[4] = { "ml", "bic", "aic", "aicc" };
          ...
          if (k == 4) {
            printf("\nError, unknown protein model selection type, you can "
                   "specify one of the following selection criteria:\n\n");
            for (k = 0; k < 4; k++)
              printf("--auto-prot=%s\n", autoModels[k]);
            printf("\n");
            errorExit(-1);
      ...
      switch (c) {
        ...
        case 'f':
          ...
          if (fOptionCount > 1) {
            printf(
              "\nError: only one of the various \"-f \" options can be used "
              "per ExaML run!\n");
        ...
        case 'm':
          ...
          if (modelExists(model, tr) == 0) {
            if (processID == 0) {
              ...
              printf(
                "For per site rates (called CAT in previous versions) use: "
                "PSR\n");
  ...
  if (adef->useQuartetGrouping && adef->mode != QUARTET_CALCULATION) {
    if (processID == 0)
      printf("\nError, you must specify \"-Y quartetGroupingFileName\" in "
  ...
  if (adef->numberRandomQuartets > 0 && adef->mode != QUARTET_CALCULATION) {
    if (processID == 0)
      printf("\nError, you must specify \"-r randomQuartetNumber\" in "
             "combination with \"-f q\"\n");
    errorExit(-1);
  }
  ...
  if ((adef->numberRandomQuartets > 0) && (adef->useQuartetGrouping)) {
    if (processID == 0)
      printf("\nError, you must specify either \"-r randomQuartetNumber\" or "
  ...
  if (tr->constraintTree) {
    if (!seedSet && processID == 0) {
      printf("\nError, you must specify a random number seed via \"-p\" when  "
             "using a constraint\n");
      printf("tree via \"-g\" \n");
      errorExit(-1);
    }
  ...
  if (!byteFileSet) {
    if (processID == 0)
      printf("\nError, you must specify a binary format data file with the "
             "\"-s\" option\n");
    errorExit(-1);
  }
  ...
  if (!modelSet) {
    if (processID == 0)
      printf("\nError, you must specify a model of rate heterogeneity with the "
             "\"-m\" option\n");
    errorExit(-1);
  }
  ...
  if (!treeSet && !adef->useCheckpoint) {
    if (processID == 0) {
      printf("\nError: please either specify a starting tree for this run with "
             "-t\n");
...
static void
printModelAndProgramInfo(tree* tr, analdef* adef, int argc, char* argv[])
{
  if (processID == 0) {
    int i, model;
    ...
    printBoth(infoFile, "\n\nThis is %s version %s released by Alexandros "
                        "Stamatakis, Andre Aberer, and Alexey Kozlov in "
                        "%s.\n\n",
              programName, programVersion, programDate);
    ...
    printBoth(infoFile, "Proportion of gaps and completely undetermined "
                        "characters in this alignment: %3.2f%s\n",
              100.0 * tr->gapyness, "%");
    ...
    if (adef->perGeneBranchLengths)
      printBoth(infoFile, "Using %d distinct models/data partitions with "
                          "individual per partition branch length "
                          "optimization\n\n\n",
                tr->NumberOfModels);
    else
      printBoth(infoFile, "Using %d distinct models/data partitions with joint "
    ...
    if (tr->rateHetModel == GAMMA || tr->rateHetModel == GAMMA_I)
      printBoth(
        infoFile,
        "%s model of rate heteorgeneity, ML estimate of alpha-parameter\n\n",
        modelType);
    else {
    ...
    for (model = 0; model < tr->NumberOfModels; model++) {
      ...
      switch (tr->partitionData[model].dataType) {
        case DNA_DATA:
          ...
          break;
        case AA_DATA:
          assert(tr->partitionData[model].protModels >= 0 &&
                 tr->partitionData[model].protModels < NUM_PROT_MODELS);
          ...
          if (!tr->partitionData[model].optimizeBaseFrequencies)
            printBoth(infoFile, "Using %s Base Frequencies\n",
                      (tr->partitionData[model].protFreqs == 1) ? "empirical"
                                                                : "fixed");
          else
            printBoth(infoFile, "ML optimization of base frequencies\n");
          break;
        ...
        /*
           ...
           secondaryModelList[tr->secondaryStructureModel]);
            break;
            case GENERIC_32:
            printBoth(infoFile, "DataType: Multi-State with %d distinct states
           in
...
void
printResult(tree* tr, analdef* adef, boolean finalPrint)
{
  if (processID == 0) {
    FILE* logFile;
    ...
    switch (adef->mode) {
      case TREE_EVALUATION:
        Tree2String(tr->tree_string, tr, tr->start->back, TRUE, TRUE, FALSE,
                    FALSE, finalPrint, SUMMARIZE_LH, FALSE, FALSE);
      ...
      case BIG_RAPID_MODE:
        if (finalPrint) {
          switch (tr->rateHetModel) {
            case GAMMA:
            case GAMMA_I:
              Tree2String(tr->tree_string, tr, tr->start->back, TRUE, TRUE,
                          FALSE, FALSE, finalPrint, SUMMARIZE_LH, FALSE, FALSE);
            ...
            case CAT:
              ...
              FALSE, FALSE, finalPrint, adef,
              NO_BRANCHES, FALSE, FALSE);*/

              Tree2String(tr->tree_string, tr, tr->start->back, TRUE, TRUE,
                          FALSE, FALSE, TRUE, SUMMARIZE_LH, FALSE, FALSE);
          ...
          Tree2String(tr->tree_string, tr, tr->start->back, FALSE, TRUE, FALSE,
                      FALSE, finalPrint, NO_BRANCHES, FALSE, FALSE);
          logFile = myfopen(temporaryFileName, "wb");
          fprintf(logFile, "%s", tr->tree_string);
          fclose(logFile);
        }
...
void
getDataTypeString(tree* tr, int model, char typeOfData[1024])
{
  switch (tr->partitionData[model].dataType) {
    case AA_DATA:
...
static void
printRatesDNA_BIN(int n, double* r, char** names, char* fileName)
{
  int i, j, c;
...
static void
printRatesRest(int n, double* r, char** names, char* fileName)
{
  int i, j, c;
...
static double
branchLength(int model, double* z, tree* tr)
{
  double x;
...
static double
treeLengthRec(nodeptr p, tree* tr, int model)
{
...
static double
treeLength(tree* tr, int model)
{
  return treeLengthRec(tr->start->back, tr, model);
}

static void
printFreqs(int n, double* f, char** names, char* fileName)
{
  int k;
...
static void
printModelParams(tree* tr, analdef* adef, int treeIteration)
{
  int model;

  double *f = (double *)NULL, *r = (double *)NULL;

  char fileName[2048], buf[64];
  ...
  for (model = 0; model < tr->NumberOfModels; model++) {
    ...
    printBothOpenDifferentFile(
      fileName,
      "Model Parameters of Partition %d, Name: %s, Type of Data: %s\n", model,
    ...
    switch (tr->partitionData[model].dataType) {
      case AA_DATA: {
        char* freqNames[20] = { "A", "R", "N ", "D", "C", "Q", "E",
                                "G", "H", "I",  "L", "K", "M", "F",
                                "P", "S", "T",  "W", "Y", "V" };
        ...
        if (tr->partitionData[model].protModels == LG4M ||
          ...
          for (k = 0; k < 4; k++) {
            ...
            printRatesRest(20, tr->partitionData[model].substRates_LG4[k],
                           freqNames, fileName);
            printBothOpenDifferentFile(fileName, "\n");
            printFreqs(20, tr->partitionData[model].frequencies_LG4[k],
                       freqNames, fileName);
          }
        }
...
static void
finalizeInfoFile(tree* tr, analdef* adef)
{
  if (processID == 0) {
    double t;
    ...
    switch (adef->mode) {
      case BIG_RAPID_MODE:
        ...
        break;
      case TREE_EVALUATION:
        printBothOpen("\n\nOverall Time for evaluating the likelihood of %d "
                      "trees: %f secs\n\n",
                      tr->numberOfTrees, t);
        printBothOpen("\n\nThe model parameters of the trees have been written "
                      "to files called %s.i\n",
                      modelFileName);
        printBothOpen("where i is the number of the tree\n\n");
        printBothOpen("Note that, in case of a restart from a checkpoint, some "
                      "tree model files will have been produced by previous "
                      "runs!\n\n");
        printBothOpen(
          "The trees with branch lengths have been written to file: %s\n",
          treeFileName);
...
static int
iterated_bitcount(unsigned int n)
{
  int count = 0;
...
static void
compute_bits_in_16bits(char* bits_in_16bits)
{
  unsigned int i;
...
unsigned int
precomputed16_bitcount(unsigned int n, char* bits_in_16bits)
{
...
static void
initializePartitions(tree* tr)
{
  size_t i, len, j, width;

  int model, maxCategories;
  ...
  for (model = 0; model < tr->NumberOfModels; model++) {
    const partitionLengths* pl =
      getPartitionLengths(&(tr->partitionData[model]));
    ...
    /*
     globalScaler needs to be 2 * tr->mxtips such that scalers of inner AND tip
     nodes can be added without a case switch
     to this end, it must also be initialized with zeros -> calloc
   */

    len = 2 * tr->mxtips;
    tr->partitionData[model].globalScaler =
      (unsigned int*)calloc(len, sizeof(unsigned int));

#ifdef _USE_OMP
    tr->partitionData[model].threadGlobalScaler =
      (unsigned int**)calloc(tr->nThreads, sizeof(unsigned int*));

    tr->partitionData[model].reductionBuffer =
      (double*)calloc(tr->nThreads, sizeof(double));
    tr->partitionData[model].reductionBuffer2 =
      (double*)calloc(tr->nThreads, sizeof(double));
    ...
    for (t = 0; t < tr->maxThreadsPerModel; ++t) {
      ...
      if (pAss) {
        ...
        tr->partitionData[model].threadGlobalScaler[tid] =
          (unsigned int*)calloc(len, sizeof(unsigned int));
      }
    }
#endif

    tr->partitionData[model].left = (double*)malloc_aligned(
      pl->leftLength * (maxCategories + 1) * sizeof(double));
    tr->partitionData[model].right = (double*)malloc_aligned(
      pl->rightLength * (maxCategories + 1) * sizeof(double));
    tr->partitionData[model].EIGN =
      (double*)malloc(pl->eignLength * sizeof(double));
    tr->partitionData[model].EV =
      (double*)malloc_aligned(pl->evLength * sizeof(double));
    tr->partitionData[model].EI =
      (double*)malloc(pl->eiLength * sizeof(double));

    tr->partitionData[model].substRates =
      (double*)malloc(pl->substRatesLength * sizeof(double));

    // must already be set as a consequence of alloc in function readPartitions
    // and the subsequent copy of bf->partitions into tr->partitions!
    assert(tr->partitionData[model].frequencies != (double*)NULL);
    // tr->partitionData[model].frequencies       =
    // (double*)malloc(pl->frequenciesLength * sizeof(double));

    tr->partitionData[model].freqExponents =
      (double*)malloc(pl->frequenciesLength * sizeof(double));
    tr->partitionData[model].empiricalFrequencies =
      (double*)malloc(pl->frequenciesLength * sizeof(double));
    tr->partitionData[model].tipVector =
      (double*)malloc_aligned(pl->tipVectorLength * sizeof(double));
    ...
    if (tr->partitionData[model].protModels == LG4M ||
      ...
      for (k = 0; k < 4; k++) {
        tr->partitionData[model].rawEIGN_LG4[k] =
          (double*)malloc(pl->eignLength * sizeof(double));
        tr->partitionData[model].EIGN_LG4[k] =
          (double*)malloc(pl->eignLength * sizeof(double));
        tr->partitionData[model].EV_LG4[k] =
          (double*)malloc_aligned(pl->evLength * sizeof(double));
        tr->partitionData[model].EI_LG4[k] =
          (double*)malloc(pl->eiLength * sizeof(double));
        tr->partitionData[model].substRates_LG4[k] =
          (double*)malloc(pl->substRatesLength * sizeof(double));
        tr->partitionData[model].frequencies_LG4[k] =
          (double*)malloc(pl->frequenciesLength * sizeof(double));
        tr->partitionData[model].tipVector_LG4[k] =
          (double*)malloc_aligned(pl->tipVectorLength * sizeof(double));
      }
    }

    tr->partitionData[model].symmetryVector =
      (int*)malloc(pl->symmetryVectorLength * sizeof(int));
    tr->partitionData[model].frequencyGrouping =
      (int*)malloc(pl->frequencyGroupingLength * sizeof(int));

    tr->partitionData[model].perSiteRates =
      (double*)malloc(sizeof(double) * tr->maxCategories);
...
#ifdef __MIC_NATIVE
    tr->partitionData[model].mic_EV =
      (double*)malloc_aligned(4 * pl->evLength * sizeof(double));
    tr->partitionData[model].mic_tipVector =
      (double*)malloc_aligned(4 * pl->tipVectorLength * sizeof(double));
    tr->partitionData[model].mic_umpLeft =
      (double*)malloc_aligned(4 * pl->tipVectorLength * sizeof(double));
    tr->partitionData[model].mic_umpRight =
      (double*)malloc_aligned(4 * pl->tipVectorLength * sizeof(double));

    /* for Xeon Phi, sumBuffer must be padded to the multiple of 8 (because of
     * site blocking in kernels) */
    ...
    tr->partitionData[model].sumBuffer =
      (double*)malloc_aligned(padded_width * span * sizeof(double));

    /* fill padding entries with 1. (will be corrected for with zero site
     * weights in wgt) */
    {
      int k;
...
#else
    tr->partitionData[model].sumBuffer = (double*)malloc_aligned(
      width * (size_t)(tr->partitionData[model].states) *
      discreteRateCategories(tr->rateHetModel) * sizeof(double));
#endif

#ifdef __CUDA
    tr->partitionData[model].cudaPackage = cudaGPAlloc(
      tr->partitionData[model].width, tr->partitionData[model].states,
      getUndetermined(tr->partitionData[model].dataType) + 1, tr->mxtips, 
      tr->partitionData[model].yResource, tr->partitionData[model].wgt);
#endif

    /* tr->partitionData[model].wgt = (int *)malloc_aligned(width *
     * sizeof(int));	   */

    /* rateCategory must be assigned using calloc() at start up there is only
     * one rate category 0 for all sites */
    ...
    if (width > 0 && tr->saveMemory) {
      ...
      tr->partitionData[model].gapVector =
        (unsigned int*)calloc(len, sizeof(unsigned int));

      tr->partitionData[model].gapColumn = (double*)malloc_aligned(
        ((size_t)tr->mxtips) * ((size_t)(tr->partitionData[model].states)) *
        discreteRateCategories(tr->rateHetModel) * sizeof(double));
    } else {
  ...
  /* set up the averaged frac changes per partition such that no further reading
   accesses to aliaswgt are necessary
...
*/
    ...
    unsigned long* modelWeights =
      (unsigned long*)calloc(tr->NumberOfModels, sizeof(unsigned long));
    ...
    MPI_Allreduce(MPI_IN_PLACE, modelWeights, tr->NumberOfModels,
    ...
    for (model = 0; model < tr->NumberOfModels; model++) {
      ...
      tr->partitionContributions[model] =
        ((double)modelWeights[model]) / ((double)wgtsum);
    }
  ...
  if (tr->saveMemory) {
    for (model = 0; model < tr->NumberOfModels; model++) {
      int undetermined = getUndetermined(tr->partitionData[model].dataType);
          ...
          for (i = 0; i < width; i++)
            if (tr->partitionData[model].yVector[j][i] == undetermined)
              tr->partitionData[model]
                .gapVector[tr->partitionData[model].gapVectorLength * j +
                           i / 32] |= mask32[i % 32];
      }
    }
  }
...
static void
initializeTree(tree* tr, analdef* adef)
{
  size_t i;
  ...
  if (NUM_BRANCHES < tr->numBranches) {
    if (processID == 0)
      printf(
        "You have specified per-partition branch lengths (-M option) with %d  models. \n\
Please set #define NUM_BRANCHES in axml.h to %d (or higher) and recompile %s\n",
        tr->NumberOfModels, tr->NumberOfModels, programName);
    error_MPI_Exit();
  }

  /* If we use the RF-based convergence criterion we will need to allocate some
   hash tables.
...
static int
getNumberOfTrees(char* fileName, boolean getOffsets, exa_off_t* treeOffsets)
{
...
static void
optimizeTrees(tree* tr, analdef* adef)
{
  exa_off_t* treeOffsets;
  ...
  tr->treeStrings = (char*)malloc(sizeof(char) * (size_t)tr->treeStringLength *
                                  (size_t)tr->numberOfTrees);
  ...
  if (adef->useCheckpoint) {
    ...
    Tree2String(tr->tree_string, tr, tr->start->back, TRUE, TRUE, FALSE, FALSE,
                FALSE, SUMMARIZE_LH, FALSE, FALSE);
    memcpy(&(tr->treeStrings[(size_t)tr->treeStringLength * (size_t)i]),
           tr->tree_string, sizeof(char) * tr->treeStringLength);
  ...
  if (processID == 0) {
    FILE* f = myfopen(treeFileName, "w");

    for (i = 0; i < tr->numberOfTrees; i++) {
      printBothOpen("Likelihood tree %d: %f \n", i, tr->likelihoods[i]);
      fprintf(f, "%s",
              &(tr->treeStrings[(size_t)tr->treeStringLength * (size_t)i]));
    }
...
static void
readByteFile(tree* tr, int commRank, int commSize)
{
  ...
  initializePartitionAssignment(&pAss, bFile->partitions, bFile->numPartitions,
                                commSize);
  assign(pAss);
...
#ifdef _USE_OMP
void
allocateXVectors(tree* tr)
{
  nodeptr p = tr->start, q = p->back;
  ...
  computeTraversalInfo(q, &(tr->td[0].ti[0]), &(tr->td[0].count), tr->mxtips,
                       tr->numBranches, FALSE);
  ...
  for (i = 1; i < tr->td[0].count; i++) {
    ...
    /* now loop over all partitions for nodes p, q, and r of the current
     * traversal vector entry */
    ...
    for (model = 0; model < tr->NumberOfModels; model++) {
      ...
      size_t
        ...
        /* get the length of the current likelihood array stored at node p.
       This is
       important mainly for the SEV-based memory saving option described in
       here:

       F. Izquierdo-Carrasco, S.A. Smith, A. Stamatakis: "Algorithms, Data
       Structures, and Numerics for Likelihood-based Phylogenetic Inference of
       Huge Trees".

       So tr->partitionData[model].xSpaceVector[i] provides the length of the
       allocated conditional array of partition model
       and node i
    */

        availableLength = tr->partitionData[model]
                            .xSpaceVector[(tInfo->pNumber - tr->mxtips - 1)],
        requiredLength = 0;

      /* memory saving stuff, not important right now, but if you are interested
       * ask Fernando */
      ...
      if (tr->saveMemory) {
        ...
        unsigned int *x1_gap = &(
                       tr->partitionData[model]
                         .gapVector[tInfo->qNumber *
                                    tr->partitionData[model].gapVectorLength]),
                     *x2_gap = &(
                       tr->partitionData[model]
                         .gapVector[tInfo->rNumber *
                                    tr->partitionData[model].gapVectorLength]),
                     *x3_gap = &(
                       tr->partitionData[model]
                         .gapVector[tInfo->pNumber *
                                    tr->partitionData[model].gapVectorLength]);

        for (j = 0; j < (size_t)tr->partitionData[model].gapVectorLength; j++) {
          x3_gap[j] = x1_gap[j] & x2_gap[j];
          setBits +=
            (size_t)(precomputed16_bitcount(x3_gap[j], tr->bits_in_16bits));
        }

        requiredLength = (width - setBits) * rateHet * states * sizeof(double);
      } else
        /* if we are not trying to save memory the space required to store an
       inner likelihood array
       is the number of sites in the partition times the number of states of the
       data type in the partition
       times the number of discrete GAMMA rates (1 for CAT essentially) times 8
       bytes */
        requiredLength = width * rateHet * states * sizeof(double);

      /* Initially, even when not using memory saving no space is allocated for
       inner likelihood arrats hence
      ...
      if (requiredLength != availableLength) {
        ...
        /* allocate memory: note that here we use a byte-boundary aligned
...
void
assignPartitionsToThreads(tree* tr, int commRank)
{
  ...
  PartitionAssignment* pAss = NULL;
  initializePartitionAssignment(&pAss, rankPartitions, tr->NumberOfModels,
                                tr->nThreads);

  /* */
  for (i = 0; i < pAss->numPartitions; ++i) {
    Partition* p = pAss->partitions + i;
...
/*
       tell the CPU to ignore exceptions generated by denormalized floating
   point values.
       If this is not done, depending on the input data, the likelihood
   functions can exhibit
    ...
    */
    ...
    /* parse command line arguments: this has a side effect on tr struct and
     * adef struct variables */
...
#ifdef _USE_OMP
    if (tr->saveMemory) {
      printBothOpen("\nError: Memory saving option \"-S\" is not supported by "
                    "the OpenMP version of ExaML!\n\n");
      error_MPI_Exit();
    }
#endif
    ...
    /* do some error checks for the LG4 model and the binary models and the MIC
     * and exit gracefully */

    {
      int countBinary = 0, countLG4 = 0, model;

#ifdef __MIC_NATIVE
      if (tr->saveMemory) {
        printBothOpen("Error: There is no MIC support yet for the memory "
                      "saving option \"-S\"!\n\n");
        error_MPI_Exit();
      }
      ...
      if (tr->rateHetModel == CAT) {
        printBothOpen(
          "Error: There is no MIC support yet for the PSR model!\n\n");
        error_MPI_Exit();
      }
#endif
      ...
      if (countLG4 > 0) {
        if (tr->saveMemory == TRUE) {
          printBothOpen("Error: the LG4 substitution model does not work in "
                        "combination with the \"-S\" memory saving flag!\n\n");
          error_MPI_Exit();
        }

        if (tr->rateHetModel == CAT) {
          printBothOpen("Error: the LG4 substitution model does not work for "
                        "proportion of invariavble sites estimates!\n\n");
          error_MPI_Exit();
        }
      }
      ...
      if (countBinary > 0) {
        if (tr->saveMemory == TRUE) {
          printBothOpen("Error: Binary data partitions can not be used in "
                        "combination with the \"-S\" memory saving flag!\n\n");
          error_MPI_Exit();
        }

#ifdef __MIC_NATIVE
        printBothOpen("Error: There is no MIC support yet for binary data "
                      "partitions!\n\n");
        error_MPI_Exit();
#endif
      }
    ...
    /*
   this will re-start ExaML exactly where it has left off from a checkpoint
   file,
   while checkpointing is important and has to be implemented for the library we
   should not worry about this right now
*/
    ...
    switch (adef->mode) {
      ...
      case BIG_RAPID_MODE:
        if (adef->useCheckpoint) {
          ...
          /* not important, only used to keep track of total accumulated exec
         time
         when checkpointing and restarts were used */
          ...
          /* get the starting tree: here we just parse the tree passed via the
     command line
     ...
     which we maybe should skip, TODO */
          ...
          /*
         here we do an initial full tree traversal on the starting tree using
         the
         Felsenstein pruning algorithm
         This should basically be the first call to the library that actually
         computes something :-)
      */

          evaluateGeneric(tr, tr->start, TRUE);

          /* the treeEvaluate() function repeatedly iterates over the entire
           * tree
           * to optimize branch lengths until convergence */
  ...
  /* return 0 which means that our unix program terminated correctly, the return
   * value is not 1 here */
