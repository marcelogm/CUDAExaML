...
static void optParamGeneric(tree* tr, double modelEpsilon, linkageList* ll,
                            int numberOfModels, int rateNumber, double lim_inf,
                            double lim_sup, int whichParameterType);
...
static void
setRateModel(tree* tr, int model, double rate, int position)
{
  ...
  if (tr->partitionData[model].nonGTR) {
    int i, index = tr->partitionData[model].symmetryVector[position],
...
static linkageList*
initLinkageList(int* linkList, tree* tr)
{
...
static linkageList*
initLinkageListString(char* linkageString, tree* tr)
{
  int *list = (int *)malloc(sizeof(int) * tr->NumberOfModels), j;

  linkageList* l;

  char *str1, *saveptr,
    *ch = (char *)calloc(strlen(linkageString), sizeof(char)), *token;
...
static void
init_Q_MatrixSymmetries(char* linkageString, tree* tr, int model)
{
  int states = tr->partitionData[model].states,
      numberOfRates = ((states * states - states) / 2),
      *list = (int *)malloc(sizeof(int) * numberOfRates), j, max = -1;
...
static linkageList*
initLinkageListGTR(tree* tr)
{
  int i, *links = (int *)malloc(sizeof(int) * tr->NumberOfModels),
         firstAA = tr->NumberOfModels + 2, countGTR = 0, countOtherModel = 0;
  linkageList* ll;
  ...
  assert((countGTR > 0 && countOtherModel == 0) ||
...
static void
freeLinkageList(linkageList* ll)
{
  int i;
...
void
scaleLG4X_EIGN(tree* tr, int model)
{
  double acc = 0.0;
  ...
  for (i = 0; i < 4; i++)
    acc += tr->partitionData[model].weights[i] *
           tr->partitionData[model].gammaRates[i];

  acc = 1.0 / acc;

  /*
  printf("update %f %f %f %f %f\n", acc, tr->partitionData[model].gammaRates[0],
  tr->partitionData[model].gammaRates[1],
  tr->partitionData[model].gammaRates[2],
  tr->partitionData[model].gammaRates[3]);

  printf("weigths: %f %f %f %f\n", tr->partitionData[model].weights[0],
  tr->partitionData[model].weights[1], tr->partitionData[model].weights[2],
  tr->partitionData[model].weights[3]);
*/

  for (i = 0; i < 4; i++)
    for (l = 0; l < 20; l++)
      tr->partitionData[model].EIGN_LG4[i][l] =
        tr->partitionData[model].rawEIGN_LG4[i][l] * acc;
}

static void
updateWeights(tree* tr, int model, int rate, double value)
{
  int j;
  ...
  for (j = 0; j < 4; j++)
    tr->partitionData[model].weights[j] =
      exp(tr->partitionData[model].weightExponents[j]) / w;
}

static void
optimizeWeights(tree* tr, double modelEpsilon, linkageList* ll,
                int numberOfModels)
{
  int i;
  ...
  // printf("W: %f %f [%f] ->", tr->perPartitionLH[0], tr->perPartitionLH[1],
  // initialLH);

  for (i = 0; i < 4; i++)
    optParamGeneric(tr, modelEpsilon, ll, numberOfModels, i, -1000000.0, 200.0,
                    LXWEIGHT_F);
  ...
  // printf("%f %f [%f]\n",  tr->perPartitionLH[0], tr->perPartitionLH[1],
  // finalLH);
}

static void
changeModelParameters(int index, int rateNumber, double value,
                      int whichParameterType, tree* tr)
{
  switch (whichParameterType) {
    ...
    case ALPHA_F:
      ...
      makeGammaCats(tr->partitionData[index].alpha,
                    tr->partitionData[index].gammaRates, 4, tr->useMedian);
      break;
    case FREQ_F: {
      int states = tr->partitionData[index].states, j;
      ...
      for (j = 0; j < states; j++)
        tr->partitionData[index].frequencies[j] =
          exp(tr->partitionData[index].freqExponents[j]) / w;
...
static void
evaluateChange(tree* tr, int rateNumber, double* value, double* result,
               boolean* converged, int whichFunction, int numberOfModels,
    ...
    if (ll->ld[i].valid) {
      if (converged[pos]) {
        ...
        for (k = 0; k < ll->ld[i].partitions; k++) {
          ...
          changeModelParameters(index, rateNumber, value[pos], whichFunction,
                                tr);
        }
      }
      pos++;
    } else {
      // if this partition is not being optimized anyway (e.g., we may be
      // optimizing GTR rates for all DNA partitions,
      // but there are also a couple of Protein partitions with fixed models
      // like WAG, JTT, etc.) set executeModel to FALSE
...
static void
brentGeneric(double* ax, double* bx, double* cx, double* fb, double tol,
             double* xmin, double* result, int numberOfModels,
             int whichFunction, int rateNumber, tree* tr, linkageList* ll,
             double* lim_inf, double* lim_sup)
{
  int iter, i;
  double *a = (double *)malloc(sizeof(double) * numberOfModels),
...
static int
brakGeneric(double* param, double* ax, double* bx, double* cx, double* fa,
            double* fb, double* fc, double* lim_inf, double* lim_sup,
            int numberOfModels, int rateNumber, int whichFunction, tree* tr,
            linkageList* ll, double modelEpsilon)
{
  double *ulim = (double *)malloc(sizeof(double) * numberOfModels),
         *u = (double *)malloc(sizeof(double) * numberOfModels),
         *r = (double *)malloc(sizeof(double) * numberOfModels),
         *q = (double *)malloc(sizeof(double) * numberOfModels),
         *fu = (double *)malloc(sizeof(double) * numberOfModels),
  ...
  int i, *state = (int *)malloc(sizeof(int) * numberOfModels),
  ...
  evaluateChange(tr, rateNumber, param, fa, converged, whichFunction,
                 numberOfModels, ll, modelEpsilon);
...
static void
optLG4X(tree* tr, double modelEpsilon, linkageList* ll, int numberOfModels)
{
  int i;

  for (i = 0; i < 4; i++) {
    optParamGeneric(tr, modelEpsilon, ll, numberOfModels, i, LG4X_RATE_MIN,
                    LG4X_RATE_MAX, LXRATE_F);
...
static void
optAlphasGeneric(tree* tr, double modelEpsilon, linkageList* ll)
{
  int i, non_LG4X_Partitions = 0, LG4X_Partitions = 0;

  /* assumes homogeneous super-partitions, that either contain DNA or AA
   * partitions !*/
  ...
  if (non_LG4X_Partitions > 0)
    optParamGeneric(tr, modelEpsilon, ll, non_LG4X_Partitions, -1, ALPHA_MIN,
                    ALPHA_MAX, ALPHA_F);
...
static double
minFreq(int index, int whichFreq, tree* tr, double absoluteMin)
{
  double min = 0.0, *w = tr->partitionData[index].freqExponents, c = 0.0;

  int states = tr->partitionData[index].states, i;
...
static double
maxFreq(int index, int whichFreq, tree* tr, double absoluteMax)
{
  double max = 0.0, *w = tr->partitionData[index].freqExponents, c = 0.0;
  ...
  max = log(1.0 - ((double)(states - 1) * FREQ_MIN)) + log(c) -
        log((double)(states - 1) * FREQ_MIN);
...
static void
optParamGeneric(tree* tr, double modelEpsilon, linkageList* ll,
                int numberOfModels, int rateNumber, double _lim_inf,
                double _lim_sup, int whichParameterType)
{
  int l, k, j, pos;

  double *startRates = (double *)malloc(sizeof(double) * numberOfModels * 4),
         *startWeights = (double *)malloc(sizeof(double) * numberOfModels * 4),
         *startExponents =
           (double *)malloc(sizeof(double) * numberOfModels * 4),
...
*/
  ...
  for (l = 0, pos = 0; l < ll->entries; l++) {
    if (ll->ld[l].valid) {
      ...
      for (j = 0; j < ll->ld[l].partitions; j++) {
        int index = ll->ld[l].partitionList[j];
        ...
        switch (whichParameterType) {
          ...
          case FREQ_F:
            ...
            startValues[pos] =
              tr->partitionData[index].freqExponents[rateNumber];
            break;
          case LXRATE_F:
            lim_inf[pos] = _lim_inf;
            lim_sup[pos] = _lim_sup;
            ...
            memcpy(&startRates[pos * 4], tr->partitionData[index].gammaRates,
                   4 * sizeof(double));
            memcpy(&startExponents[pos * 4],
                   tr->partitionData[index].weightExponents,
                   4 * sizeof(double));
            memcpy(&startWeights[pos * 4], tr->partitionData[index].weights,
                   4 * sizeof(double));
            break;
          case LXWEIGHT_F:
            lim_inf[pos] = _lim_inf;
            ...
            assert(rateNumber >= 0 && rateNumber < 4);
            startValues[pos] =
              tr->partitionData[index].weightExponents[rateNumber];
            break;
          default:
            assert(0);
        }
  ...
  brakGeneric(_param, _a, _b, _c, _fa, _fb, _fc, lim_inf, lim_sup,
              numberOfModels, rateNumber, whichParameterType, tr, ll,
              modelEpsilon);
  ...
  brentGeneric(_a, _b, _c, _fb, modelEpsilon, _x, endLH, numberOfModels,
               whichParameterType, rateNumber, tr, ll, lim_inf, lim_sup);

  for (k = 0, pos = 0; k < ll->entries; k++) {
    if (ll->ld[k].valid) {
      if (startLH[pos] > endLH[pos]) {
        // if the initial likelihood was better than the likelihodo after
        // optimization, we set the values back
        // to their original values

        for (j = 0; j < ll->ld[k].partitions; j++) {
          int index = ll->ld[k].partitionList[j];

          changeModelParameters(index, rateNumber, startValues[pos],
                                whichParameterType, tr);
        }
      } else {
        ...
        for (j = 0; j < ll->ld[k].partitions; j++) {
          ...
          changeModelParameters(index, rateNumber, _x[pos], whichParameterType,
                                tr);
        }
      }
      pos++;
  ...
  // LIBRARY call the barrier here in the LIBRARY to update model params at all
  // threads/processes !
...
static void
optFreqs(tree* tr, double modelEpsilon, linkageList* ll, int numberOfModels,
         int states)
{
  int rateNumber;

  double freqMin = -1000000.0, freqMax = 200.0;

  for (rateNumber = 0; rateNumber < states; rateNumber++)
    optParamGeneric(tr, modelEpsilon, ll, numberOfModels, rateNumber, freqMin,
                    freqMax, FREQ_F);
}

static void
optBaseFreqs(tree* tr, double modelEpsilon, linkageList* ll)
{
  int i, states, dnaPartitions = 0, aaPartitions = 0, binaryPartitions = 0;
  ...
  for (i = 0; i < ll->entries; i++) {
    switch (tr->partitionData[ll->ld[i].partitionList[0]].dataType) {
      case DNA_DATA:
        ...
        if (tr->partitionData[ll->ld[i].partitionList[0]]
              .optimizeBaseFrequencies) {
          ll->ld[i].valid = TRUE;
          dnaPartitions++;
        } else
          ll->ld[i].valid = FALSE;
...
// new version for optimizing rates, an external loop that iterates over the
// rates

static void
optRates(tree* tr, double modelEpsilon, linkageList* ll, int numberOfModels,
         int states)
{
  int rateNumber, numberOfRates = ((states * states - states) / 2) - 1;

  for (rateNumber = 0; rateNumber < numberOfRates; rateNumber++)
    optParamGeneric(tr, modelEpsilon, ll, numberOfModels, rateNumber, RATE_MIN,
                    RATE_MAX, RATE_F);
}
...
static void
optRatesGeneric(tree* tr, double modelEpsilon, linkageList* ll)
{
  int i, dnaPartitions = 0, aaPartitions = 0, states = -1;
...
static int
catCompare(const void* p1, const void* p2)
{
...
static void
categorizePartition(
  tree* tr, rateCategorize* rc, int model, int lower, int upper, double* patrat,
...
static void
optRateCatPthreads(tree* tr, double lower_spacing, double upper_spacing)
{
#ifdef _USE_OMP
#pragma omp parallel
#endif
  {
    ...
    for (m = 0; m < maxModel; m++) {
      /* just defaults -> if partion wasn't assigned to this thread, it will be
       * ignored later on */
      ...
      pInfo* partition = &(tr->partitionData[model]);
      ...
      for (i = offset; i < offset + width; ++i) {
        ...
        while ((initialRate - k * lower_spacing > 0.0001) &&
               ((v = evaluatePartialGeneric(
                   tr, i, initialRate - k * lower_spacing, model)) > leftLH) &&
        ...
        while (((v = evaluatePartialGeneric(
                   tr, i, initialRate + k * upper_spacing, model)) > rightLH) &&
...
*/
static void
getWeightsAndWeightedRates(const tree* const tr, int** weightPerPart_result,
                           double** weightedRates_result)
{
  int i, *weightPerPart = (int *)NULL;

  double* weightedRates = (double*)NULL;

  *weightPerPart_result = (int*)calloc((size_t)tr->NumberOfModels, sizeof(int));
  *weightedRates_result =
    (double*)calloc((size_t)tr->NumberOfModels, sizeof(double));
  ...
  for (i = 0; i < tr->NumberOfModels; ++i) {
    ...
    pInfo* partition = &(tr->partitionData[i]);

    for (j = 0; j < partition->width; ++j) {
      int c = partition->rateCategory[j];

      weightPerPart[i] += partition->wgt[j];
      assert(0 <= c && c < tr->maxCategories);
      weightedRates[i] +=
        ((double)partition->wgt[j]) * partition->perSiteRates[c];
    }
  }

  MPI_Allreduce(MPI_IN_PLACE, weightPerPart, tr->NumberOfModels, MPI_INT,
                MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, weightedRates, tr->NumberOfModels, MPI_DOUBLE,
                MPI_SUM, MPI_COMM_WORLD);
...
void
checkPerSiteRates(const tree* const tr)
{
  int i, *weightPerPart = (int *)NULL;
...
/*
  ...
  Output:
  optRates_result  -- (only at master) a pointer to an array of optimized rates
  (corresponds to what used to be  tr->patratStored)
  lnls_result -- (only at master) a pointer to an array with persite-lnls that
  correspond to the newly proposed rate  (used to be tr->lhs)
 */
static void
gatherOptimizedRates(tree* tr, double** optRates_result, double** lnls_result)
{
  /* determine counts and displacement for data for each processor  */
  int *numPerProc = (int *)NULL, *displPerProc = (int *)NULL;

  calculateLengthAndDisplPerProcess(tr, &numPerProc, &displPerProc);

  gatherDistributedArray(tr, (void**)optRates_result, tr->patrat_basePtr,
                         MPI_DOUBLE, numPerProc, displPerProc);
  gatherDistributedArray(tr, (void**)lnls_result, tr->lhs_basePtr, MPI_DOUBLE,
                         numPerProc, displPerProc);
...
/*
   ...
   Output:
   rateCategory_result  -- a pointer to a global array of rate categories (used
   to be tr->rateCategory)
 ...
 */
static void
categorizeTheRates(tree* tr, double* patrat, double* lnls, int maxCategories,
                   int** rateCategory_result)
{
  int model, i;
  ...
  *rateCategory_result =
  ...
  for (model = 0; model < tr->NumberOfModels; model++) {
    ...
    rateCategorize* rc =
      (rateCategorize*)malloc(sizeof(rateCategorize) * width);
    ...
    if (where < maxCategories) {
      ...
      categorizePartition(tr, rc, model, lower, upper, patrat,
                          *rateCategory_result);
    } else {
...
*/
static void
scatterProcessedRates(tree* tr, int* rateCategory)
{
  int i,
    *countPerProc = (int *)NULL, *displPerProc = (int *)NULL,
  ...
  for (i = 0; i < tr->NumberOfModels; ++i)
    MPI_Bcast(tr->partitionData[i].perSiteRates, tr->maxCategories, MPI_DOUBLE,
              0, MPI_COMM_WORLD);
  ...
  scatterDistrbutedArray(tr, rateCategory, tr->rateCategory_basePtr, MPI_INT,
                         countPerProc, displPerProc);
 ...
 */
static void
backupRates(tree* tr, RateBackup** resultPtr)
{
  int i, numCat = tr->maxCategories;

  RateBackup* backup;

  *resultPtr =
    (RateBackup*)calloc((size_t)tr->NumberOfModels, sizeof(RateBackup));

  backup = *resultPtr;

  for (i = 0; i < tr->NumberOfModels; ++i) {
    pInfo* partition = &(tr->partitionData[i]);
    RateBackup* bk = backup + i;
    ...
    memcpy(bk->patrat, partition->patrat,
           sizeof(double) * (size_t)partition->width);
    memcpy(bk->perSiteRates, partition->perSiteRates,
           sizeof(double) * (size_t)numCat);
    memcpy(bk->rateCategory, partition->rateCategory,
           sizeof(int) * (size_t)partition->width);
  }
}

static void
restoreBackupRates(tree* tr, RateBackup* rb)
{
  int numCat = tr->maxCategories, i;
  ...
  for (i = 0; i < tr->NumberOfModels; ++i) {
    ...
    memcpy(partition->patrat, bk->patrat,
           sizeof(double) * (size_t)partition->width);
    memcpy(partition->perSiteRates, bk->perSiteRates,
           sizeof(double) * (size_t)numCat);
    memcpy(partition->rateCategory, bk->rateCategory,
           sizeof(int) * (size_t)partition->width);
  }
}

static void
deleteBackupRates(tree* tr, RateBackup** rbPtr)
{
  int i;

  for (i = 0; i < tr->NumberOfModels; ++i) {
    RateBackup* rb = &((*rbPtr)[i]);
...
static void
optimizeRateCategories(tree* tr, int _maxCategories)
{
  ...
  double lower_spacing, upper_spacing, initialLH = tr->likelihood,
                                       *optRates = (double *)NULL,
                                       *lnls = (double *)NULL;

  int *rateCategory = (int *)NULL, maxCategories = _maxCategories;

  RateBackup* rateBackup = (RateBackup*)NULL;
...
static void
printAAmatrix(tree* tr, double epsilon)
{
  if (AAisGTR(tr)) {
    int model;
...
static void
optModel(tree* tr, int numProteinModels, int* bestIndex, double* bestScores,
         boolean empiricalFreqs)
{
  int i, model;
  ...
  for (i = 0; i < numProteinModels; i++) {
    ...
    // TRUE)?"empirical":"fixed", tr->likelihood, tr->perPartitionLH[0]);
...
static void
autoProtein(tree* tr, analdef* adef)
{
  int countAutos = 0, model;
  ...
  if (countAutos > 0) {
    ...
    boolean* oldFreqs = (boolean*)malloc(sizeof(boolean) * tr->NumberOfModels);
    ...
    topolRELL_LIST* rl = (topolRELL_LIST*)malloc(sizeof(topolRELL_LIST));
    ...
    printBothOpen(
      "Automatic protein model assignment algorithm using %s criterion:\n\n",
      autoModels[tr->autoProteinSelectionType]);

    for (model = 0; model < tr->NumberOfModels; model++) {
      if (tr->partitionData[model].protModels == AUTO) {
        int bestIndexFixed = bestIndex[model],
            bestIndexEmp = bestIndexEmpFreqs[model];

        double bestLhFixed = bestScores[model],
               bestLhEmp = bestScoresEmpFreqs[model], samples = 0.0,
               freeParamsFixed = 0.0, freeParamsEmp = 0.0;
        ...
        switch (tr->rateHetModel) {
          case CAT:
            freeParamsFixed +=
              (double)tr->partitionData[model].numberOfCategories;
            freeParamsEmp +=
              (double)tr->partitionData[model].numberOfCategories;
            break;
          case GAMMA:
        ...
        switch (tr->autoProteinSelectionType) {
          ...
          case AUTO_BIC: {
            // BIC: -2 * lnL + k * ln(n)
            double bicFixed =
                     -2.0 * bestLhFixed + freeParamsFixed * log(samples),
          ...
          case AUTO_AICC: {
            ...
            /*
             * Even though samples and freeParamsFixed are fp variables, they
             * are
             * actually integers.
            ...
            if (fabs(samples - freeParamsFixed - 1.0) < 0.5)
              ...
              aiccFixed = (2.0 * (freeParamsFixed - bestLhFixed)) +
                          ((2.0 * freeParamsFixed * (freeParamsFixed + 1.0)) /
        ...
        initReversibleGTR(tr, model);
        printBothOpen(
          "\tPartition: %d best-scoring AA model: %s likelihood %f with %s "
          "base frequencies\n",
...
static void
checkMatrixSymnmetriesAndLinkage(tree* tr, linkageList* ll)
{
  int i;
  ...
  for (i = 0; i < ll->entries; i++) {
    ...
    if (partitions > 1) {
      ...
      for (k = 1; k < partitions; k++) {
        ...
        int states = tr->partitionData[index].states,
            rates = ((states * states - states) / 2);

        if (tr->partitionData[reference].nonGTR !=
            tr->partitionData[index].nonGTR)
          assert(0);
        ...
        if (tr->partitionData[reference].nonGTR) {
          ...
          for (j = 0; j < rates; j++) {
            if (tr->partitionData[reference].symmetryVector[j] !=
                tr->partitionData[index].symmetryVector[j])
              assert(0);
          }
        }
      }
...
static void
checkTolerance(double l1, double l2)
{
  if (l1 < l2) {
    double tolerance = fabs(MAX(l1, l2) * 0.000000000001);

    if (fabs(l1 - l2) > MIN(0.1, tolerance)) {
      printf("Likelihood problem in model optimization l1: %1.40f l2: %1.40f "
             "tolerance: %1.40f\n",
             l1, l2, tolerance);
      assert(0);
    }
...
void
modOpt(tree* tr, double likelihoodEpsilon, analdef* adef, int treeIteration)
{
  int i, catOpt = 0,
         *unlinked = (int *)malloc(sizeof(int) * tr->NumberOfModels);

  double inputLikelihood, currentLikelihood, modelEpsilon = 0.0001;

  linkageList *alphaList, *rateList, *freqList;
  ...
  if (0) {
    ...
    // function that checks that partitions that have linked Q matrices as in
    // our example above
    // will not have different configurations of the Q matrix as set by the
    // init_Q_MatrixSymmetries() function
    // e.g., on would have HKY and one would have GTR, while the user claimes
