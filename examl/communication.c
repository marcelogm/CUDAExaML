...
*/
void
calculateLengthAndDisplPerProcess(tree* tr, int** length_result,
                                  int** disp_result)
{
  int i;
...
static size_t
mapMpiTypeToSize(MPI_Datatype type)
{
  if (type == MPI_INT)
    return sizeof(int);
...
*/
void
scatterDistrbutedArray(tree* tr, void* src, void* destination,
                       MPI_Datatype type, int* countPerProc, int* displPerProc)
{
  int i;

  size_t typeLen = mapMpiTypeToSize(type);

  char* srcReordered = (char*)NULL;
  ...
  if (processID == 0) {
    ...
    while (aIter != aEnd) {
      ...
      memcpy(srcReordered +
               ((size_t)displPerProc[aIter->procId] +
                (size_t)seenPerProcesses[aIter->procId]) *
                 typeLen,
  ...
  MPI_Scatterv(srcReordered, countPerProc, displPerProc, type, destination,
               countPerProc[processID], type, 0, MPI_COMM_WORLD);
                       ...
                       MPI_Datatype type, int* countPerProc, int* displPerProc)
{
  ...
  MPI_Gatherv(src, countPerProc[processID], type, destinationUnordered,
              countPerProc, displPerProc, type, 0, MPI_COMM_WORLD);
 ...
 */
  if (processID == 0) {
    int i, *seenPerProcesses = (int *)calloc(processes, sizeof(int));
