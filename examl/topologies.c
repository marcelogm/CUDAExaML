...
static void
saveTopolRELLRec(tree* tr, nodeptr p, topolRELL* tpl, int* i, int numsp,
                 int numBranches)
{
  int k;
...
static void
saveTopolRELL(tree* tr, topolRELL* tpl)
{
  nodeptr p = tr->start;
  int k, i = 0;
...
static void
restoreTopolRELL(tree* tr, topolRELL* tpl)
{
  int i;
...
void
initTL(topolRELL_LIST* rl, tree* tr, int n)
{
  int i;
  ...
  for (i = 0; i < n; i++) {
    ...
    rl->t[i]->connect =
      (connectRELL*)malloc((2 * tr->mxtips - 3) * sizeof(connectRELL));
    rl->t[i]->likelihood = unlikely;
  }
}

void
freeTL(topolRELL_LIST* rl)
{
  int i;
  for (i = 0; i < rl->max; i++) {
    free(rl->t[i]->connect);
...
void
restoreTL(topolRELL_LIST* rl, tree* tr, int n)
{
  assert(n >= 0 && n < rl->max);
...
void
saveTL(topolRELL_LIST* rl, tree* tr, int index)
{
...
static int
saveSubtree(nodeptr p, topol* tpl, int numsp, int numBranches)
{
  connptr r, r0;
  nodeptr q, s;
  int t, t0, t1, k;
  ...
  if (isTip(q->number, numsp)) {
    ...
    do {
      t =
        saveSubtree(s, tpl, numsp, numBranches); /* Generate child's subtree */
...
static nodeptr
minSubtreeTip(nodeptr p0, int numsp)
{
...
static nodeptr
minTreeTip(nodeptr p, int numsp)
{
  nodeptr minp, minpb;
 ...
 */
{
  ...
  r = tpl->links + saveSubtree(minTreeTip(tr->start, tr->mxtips), tpl,
                               tr->mxtips, tr->numBranches); /* Save tree */
  r->sibling = 0;
...
int
initBestTree(bestlist* bt, int newkeep, int numsp)
{ /* initBestTree */
  int i;
  ...
  for (i = bt->ninit + 1; i <= newkeep; i++) {
    if (!(bt->byScore[i] = setupTopol(numsp)))
      break;
...
boolean
freeBestTree(bestlist* bt)
{ /* freeBestTree */
  while (bt->ninit >= 0)
    freeTopol(bt->byScore[(bt->ninit)--]);
...
static int
cmpSubtopol(connptr p10, connptr p1, connptr p20, connptr p2)
{
  connptr p1d, p2d;
  int cmp;
  ...
  if (!p1->descend)
    return -1; /* p1 = tip, p2 = node */
  if (!p2->descend)
    return 1; /* p2 = tip, p1 = node */

  p1d = p10 + p1->descend;
  p2d = p20 + p2->descend;
  while (1) { /* Two nodes */
    if ((cmp = cmpSubtopol(p10, p1d, p20, p2d)))
      return cmp; /* Subtrees */
    if (!p1d->sibling && !p2d->sibling)
      return 0; /* Lists done */
    if (!p1d->sibling)
      return -1; /* One done, other not */
    if (!p2d->sibling)
      return 1;               /* One done, other not */
...
static int
cmpTplScore(void* tpl1, void* tpl2)
{
  double l1, l2;
...
static int
findInList(void* item, void* list[], int n, int (*cmpFunc)(void*, void*))
{
  int mid, hi, lo, cmp = 0;

  if (n < 1)
    return -1; /*  No match; first index  */
  ...
  if (cmp > 0)
    lo++; /*  Result of step = 0 test  */
  return -lo;
}

static int
findTreeInList(bestlist* bt, tree* tr)
{
  topol* tpl;

  tpl = bt->byScore[0];
  saveTree(tr, tpl);
  return findInList((void*)tpl, (void**)(&(bt->byTopol[1])), bt->nvalid,
                    cmpTopol);
}

int
saveBestTree(bestlist* bt, tree* tr, boolean keepIdenticalTrees)
{
  topol *tpl, *reuse;
  ...
  scrNum =
    findInList((void*)tpl, (void**)(&(bt->byScore[1])), oldValid, cmpTplScore);
  scrNum = ABS(scrNum);
...
int
recallBestTree(bestlist* bt, int rank, tree* tr)
{
  if (rank < 1)
    rank = 1;
  if (rank > bt->nvalid)
    rank = bt->nvalid;
  if (rank > 0)
    if (!restoreTree(bt->byScore[rank], tr))
      return FALSE;
  return rank;
}
