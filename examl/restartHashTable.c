...
static boolean
treeNeedString(const char* fp, char c1, int* position)
{
  char c2 = fp[(*position)++];
  ...
  if (c2 == c1)
    ...
    int lower = MAX(0, *position - 20), upper = *position + 20;
...
static boolean
treeGetLabelString(const char* fp, char* lblPtr, int maxlen, int* position)
{
  char ch;

  boolean done, lblfound;
...
static boolean
treeFlushLabelString(const char* fp, int* position)
{
  return treeGetLabelString(fp, (char*)NULL, (int)0, position);
}

static boolean
treeProcessLengthString(const char* fp, double* dptr, int* position)
{
  (*position)++;
...
static int
treeFlushLenString(const char* fp, int* position)
{
  double dummy;
...
static int
treeFindTipByLabelString(char* str, tree* tr)
{
...
static int
treeFindTipNameString(const char* fp, tree* tr, int* position)
{
  char str[nmlngth + 2];
  int n;
...
static boolean
addElementLenString(const char* fp, tree* tr, nodeptr p, int* position)
{
  nodeptr q;
...
void
treeReadTopologyString(char* treeString, tree* tr)
{
  char* fp = treeString;
