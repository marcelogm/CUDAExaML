...
#define READ_ARRAY(file, arrPtr, numElem, size)                                \
  assert(fread(arrPtr, size, numElem, file) == (unsigned int)numElem)
...
seekPos(ByteFile* bf, int pos)
{
  ...
  switch (pos) {
    case ALN_ALIGNMENT: /* skips partitions */
    {
      ...
      toSkip += bf->numPartitions *
                (sizeof(p.states) + sizeof(p.maxTipStates) + sizeof(p.lower) +
                 sizeof(p.upper) + sizeof(p.width) + sizeof(p.dataType) +
                 sizeof(p.protModels) + sizeof(p.protFreqs) + sizeof(p.nonGTR) +
                 sizeof(p.optimizeBaseFrequencies));
    ...
    {
      toSkip += sizeof(bf->numTax) + sizeof(bf->numPattern) +
                sizeof(bf->numPartitions) + sizeof(bf->gappyness);
    }
    case ALN_HEAD:
      toSkip +=
        (3 *
         sizeof(int)); /* skips the initial int that tells us how many bytes a
                          size_t has as well as the integer for the version
                          number and the magic integer number */
      break;
    default:
      assert(0);
  }
 ...
 */
void
initializeByteFile(ByteFile** bf, char* name)
{
  *bf = (ByteFile*)calloc(1, sizeof(ByteFile));
  ByteFile* result = *bf;
  result->fh = myfopen(name, "rb");

  int sizeOfSizeT = 0, version = 0, magicNumber = 0;

  READ_VAR(result->fh, sizeOfSizeT);

  if (sizeOfSizeT != sizeof(size_t)) {
    if (processID == 0) {
      printf("\nError: the address data type has a size of %d bits on the "
             "current system while on the system on which you created the "
             "binary alignment file using the parser the address size is %d "
             "bits!\n",
             8 * (int)sizeof(size_t), 8 * sizeOfSizeT);
      printf("Usually this indicates that the parser was executed on a 32-bit "
             "system while you are trying to run ExaML on a 64-bit system.\n");
      printf("Please parse the binary alignment file on the same hardware on "
             "which you intend to run ExaML.\n\n\n");
    }
  ...
  if (version != (int)programVersionInt) {
    if (processID == 0) {
      printf("\nError: Version number %d of ExaML parser and version number %d "
             "of ExaML don't match.\n",
             version, (int)programVersionInt);
      printf("You are either using an outdated version of the parser or of "
             "ExaML.\n");
  ...
  if (magicNumber != 6517718) {
    if (processID == 0) {
      printf("\nError: The magic number %d of ExaML parser and magic number %d "
             "of ExaML don't match.\n",
             magicNumber, 6517718);
 ...
 */
void
readMyData(ByteFile* bf, PartitionAssignment* pa, int procId)
{
  seekPos(bf, ALN_ALIGNMENT);

  exa_off_t alnPos = exa_ftell(bf->fh);
  ...
  for (i = 0; i < numAssign; ++i) {
    ...
    partition->yResource =
      (unsigned char*)malloc_aligned(len * sizeof(unsigned char));
    memset(partition->yResource, 0, len * sizeof(unsigned char));
    partition->yVector =
      (unsigned char**)calloc(bf->numTax + 1, sizeof(unsigned char*));
    for (j = 1; j <= bf->numTax; ++j)
      partition->yVector[j] = partition->yResource + (j - 1) * a.width;

#ifdef OLD_LAYOUT
    for (j = 1; j <= bf->numTax; ++j) {
      exa_off_t pos = alnPos +
                      (bf->numPattern * (j - 1) + partition->lower + a.offset) *
    ...
    if (a.width == (partition->upper - partition->lower)) {
      exa_off_t pos =
        alnPos + (partition->lower * bf->numTax) * sizeof(unsigned char);

      assert(alnPos <= pos);
      exa_fseek(bf->fh, pos, SEEK_SET);
      READ_ARRAY(bf->fh, partition->yResource, a.width * bf->numTax,
                 sizeof(unsigned char));
    } else {
      for (j = 1; j <= bf->numTax; ++j) {
        exa_off_t pos =
          ...
          sizeof(unsigned char) *
            ((partition->lower * bf->numTax) /* until start of partition  */
             + ((j - 1) * (partition->upper -
                           partition->lower)) /* until start of sequence of
                                                 taxon within partition */
             + a.offset);

        assert(alnPos <= pos);
        exa_fseek(bf->fh, pos, SEEK_SET);
        READ_ARRAY(bf->fh, partition->yVector[j], a.width,
                   sizeof(unsigned char));
      }
    }
#endif
  ...
  exa_off_t wgtPos = exa_ftell(bf->fh);
  assert(!(wgtPos < 0));
...
#ifdef __MIC_NATIVE
    /* for Xeon Phi, wgt must be padded to the multiple of 8 (because of site
     * blocking in kernels) */
 ...
 */
void
initializeTreeFromByteFile(ByteFile* bf, tree* tr)
{
  assert((bf->hasRead & ALN_HEAD) && (bf->hasRead & ALN_WEIGHTS) &&
         (bf->hasRead & ALN_TAXA) && (bf->hasRead & ALN_PARTITIONS) &&
         (bf->hasRead & ALN_ALIGNMENT));
  ...
  for (i = 1; i <= bf->numTax; ++i) {
    tr->nameList[i] =
      (char*)calloc(strlen(bf->taxaNames[i - 1]) + 1, sizeof(char));
 ...
 */
  ...
  for (i = 0; i < tr->NumberOfModels; ++i) {
    ...
    assert(bf->partitions[i]->wgt != (int*)NULL ||
           bf->partitions[i]->width == 0);
    assert((tr->partitionData[i].wgt != (int*)NULL) ||
           (tr->partitionData[i].width == 0));
  }
}
