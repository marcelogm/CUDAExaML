...
/* the set of functions in here computes the log likelihood at a given branch
 * (the virtual root of a tree) */
...
/*
   ...
   the total log likelihood over the partial log lieklihoods for the sites that
   each thread has computed

   NumberOfThreads is just the number of threads.

   Note the volatile modifier here, that guarantees that the compiler will not
   do weird optimizations
   rearraengements of the code accessing those variables, because it does not
   know that several concurrent threads
...
static void calcDiagptable(const double z, const int states,
  ...
  /* transform the root branch length to the log and check if it is not too
   * small */
...
static double evaluateGAMMA_FLEX(int *wptr, double *x1_start, double *x2_start,
  ...
  /* span is the offset within the likelihood array at an inner node that gets
   us from the values
   of site i to the values of site i + 1 */

  const int span = states * 4;

  /* we distingusih between two cases here: one node of the two nodes defining
   the branch at which we put the virtual root is
   a tip. Both nodes can not be tips because we do not allow for two-taxon trees
   ;-)
   Nota that, if a node is a tip, this will always be tipX1. This is done for
   code simplicity and the flipping of the nodes
...
*/
  ...
  if (tipX1) {
    ...
    for (i = 0; i < n; i++) {
      ...
      /* take the log of the likelihood and multiply the per-gamma rate
       likelihood by 1/4.
       Under the GAMMA model the 4 discrete GAMMA rates all have the same
       probability
       of 0.25 */
    ...
    for (i = 0; i < n; i++) {
      /* same as before, only that now we access two inner likelihood vectors x1
       * and x2 */
...
static double evaluateCAT_FLEX(int *cptr, int *wptr, double *x1, double *x2,
  ...
  /* chosing between tip vectors and non tip vectors is identical in all flavors
   of this function ,regardless
  ...
  if (tipX1) {
    ...
    */
      ...
      /* similar to gamma, with the only difference that we do not integrate
       (sum)
      ...
      /*
       ...
       into one site pattern if they are exactly identical AND evolve under the
       same model,
...
/* below are the function headers for unreadeble highly optimized versions of
   the above functions
   for DNA and protein data that also use SSE3 intrinsics and implement some
   ...
   To decide which set of function implementations to use you will have to
   undefine or define _OPTIMIZED_FUNCTIONS
   in the Makefile
*/
...
void evaluateIterative(tree *tr) {
  ...
  int pNumber = tr->td[0].ti[0].pNumber, qNumber = tr->td[0].ti[0].qNumber;

  /* before we can compute the likelihood at the virtual root, we need to do a
   partial or full tree traversal to compute
   the conditional likelihoods of the vectors as specified in the traversal
   descriptor. Maintaining this tarversal descriptor consistent
   will unfortunately be the responsibility of users. This is tricky, if as
   planned for here, we use a rooted view (described somewhere in Felsenstein's
   book)
...
#pragma omp parallel for
  ...
  for (m = 0; m < tr->NumberOfModels; m++) {
    ...
    int categories, states = tr->partitionData[m].states;

    double z, *rateCategories, *diagptable = tr->partitionData[m].left;

    /* if we are using a per-partition branch length estimate, the branch has an
       index, otherwise, for a joint branch length
       estimate over all partitions we just use the branch length value with
       index 0 */
    if (tr->numBranches > 1)
      z = pz[m];
    else
    ...
    /*
         ...
         the number of site-wise rate categories can vary in the course of
       computations
    ...
    if (tr->partitionData[m].protModels == LG4M ||
        tr->partitionData[m].protModels == LG4X)
      calcDiagptableFlex_LG4(z, 4, tr->partitionData[m].gammaRates,
                             tr->partitionData[m].EIGN_LG4, diagptable, 20);
    else
      calcDiagptable(z, states, categories, rateCategories,
                     tr->partitionData[m].EIGN, diagptable);
  }

/* after the above call we are sure that we have properly and consistently
   computed the
     conditionals to the right and left of the virtual root and we can now
   invoke the
     the log likelihood computation */

/* we need to loop over all partitions. Note that we may have a mix of DNA,
 * protein binary data etc partitions */
#ifdef _USE_OMP
#pragma omp parallel
#endif
  {
    ...
    for (m = 0; m < maxModel; m++) {
      ...
      /* set this pointer to the memory area where space has been reserved a
       priori for storing the
   ...
   The reasons why this is important in terms of performance are given in this
   paper
   here which you should actually read:

   A. Stamatakis, M. Ott: "Load Balance in the Phylogenetic Likelihood Kernel".
   Proceedings of ICPP 2009, accepted for publication, Vienna, Austria,
   September 2009

   The width > 0 check is for checking if under the cyclic data distribution of
   per-partition sites to threads this thread does indeed have a site
 ...
 */
      ...
      if (tr->td[0].executeModel[model] && width > 0) {
        ...
        double partitionLikelihood = 0.0,
               ...
               *x1_gapColumn = (double *)NULL, *x2_gapColumn = (double *)NULL;
...
space for additional pointers */
        ...
        /* generic slow functions, memory saving option is not implemented for
         * these */
     ...
     implementation
     for each rate heterogeneity and data type combination, we switch over the
     number of states
     and the rate heterogeneity model */
        ...
        /* now here is a nasty part, for each partition and each node we
     maintain an integer counter to count how often
     how many entries per node were scaled by a constant factor. Here we use
     this information generated during Felsenstein's
     pruning algorithm by the newview() functions to undo the preceding scaling
     multiplications at the root, for mathematical details
     you should actually read:

     A. Stamatakis: "Orchestrating the Phylogenetic Likelihood Function on
     Emerging Parallel Architectures".
     In B. Schmidt, editor, Bioinformatics: High Performance Parallel Computer
     Architectures, 85-115, CRC Press, Taylor & Francis, 2010.
  ...
  */
        ...
        /* now we have the correct log likelihood for the current partition
         * after undoing scaling multiplications */

        /* finally, we also store the per partition log likelihood which is
     important for optimizing the alpha parameter
...
void evaluateGeneric(tree *tr, nodeptr p, boolean fullTraversal) {
  ...
  /* copy the branch lengths of the tree into the first entry of the traversal
   descriptor.
  ...
  /* now compute how many conditionals must be re-computed/re-oriented by
   newview
   to be able to calculate the likelihood at the root defined by p and q.
*/

  /* one entry in the traversal descriptor is already used, hence set the
   * tarversal length counter to 1 */
  tr->td[0].count = 1;
  ...
  if (fullTraversal) {
    ...
    if (!p->x)
      computeTraversalInfo(p, &(tr->td[0].ti[0]), &(tr->td[0].count),
                           tr->mxtips, tr->numBranches, TRUE);

    /* recompute/reorient any descriptors at or below q ?
     computeTraversalInfo computes and stores the newview() to be executed for
     the traversal descriptor */

    if (!q->x)
      computeTraversalInfo(q, &(tr->td[0].ti[0]), &(tr->td[0].count),
                           tr->mxtips, tr->numBranches, TRUE);
  }

  /* now we copy this partition execute mask into the traversal descriptor which
     must come from the
       calling program, the logic of this should not form part of the library */

  storeExecuteMaskInTraversalDescriptor(tr);

  /* also store in the traversal descriptor that something has changed i.e., in
   the parallel case that the
...
#else
    MPI_Reduce(tr->perPartitionLH, recv, tr->NumberOfModels, MPI_DOUBLE,
