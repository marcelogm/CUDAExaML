...
void
updateModel_MIC(pInfo* part)
{
  double *EV = part->EV, *tipVector = part->tipVector, *aEV = part->mic_EV,
...
#pragma ivdep
    for (int l = 0; l < states; l++) {
      aTipVector[k * span + l] = aTipVector[k * span + states + l] =
        aTipVector[k * span + 2 * states + l] =
          aTipVector[k * span + 3 * states + l] = tipVector[k * states + l];
    }
  }
}
...
void
makeP_DNA_MIC(double z1, double z2, double* rptr, double* EI, double* EIGN,
...
void
precomputeTips_DNA_MIC(int tipCase, double* tipVector, double* left,
                       ...
                       int numberOfCategories)
{
  ...
  const int span = states * 4, umpSize = span * 16;
...
inline void
mic_fma4x16(const double* inv, double* outv, double* mulv)
{
  ...
  for (int k = 0; k < 4; k++) {
    t = _mm512_mask_extload_pd(t, k1, &inv[0 + k], _MM_UPCONV_PD_NONE,
                               _MM_BROADCAST_1X8, _MM_HINT_NONE);
    t = _mm512_mask_extload_pd(t, k2, &inv[4 + k], _MM_UPCONV_PD_NONE,
                               _MM_BROADCAST_1X8, _MM_HINT_NONE);

    __m512d m = _mm512_load_pd(&mulv[k * 16]);
    acc1 = _mm512_fmadd_pd(t, m, acc1);

    t = _mm512_mask_extload_pd(t, k1, &inv[8 + k], _MM_UPCONV_PD_NONE,
                               _MM_BROADCAST_1X8, _MM_HINT_NONE);
    t = _mm512_mask_extload_pd(t, k2, &inv[12 + k], _MM_UPCONV_PD_NONE,
                               _MM_BROADCAST_1X8, _MM_HINT_NONE);
...
void
newviewGTRGAMMA_MIC(int tipCase, double* x1, double* x2, double* x3,
                    double* extEV, double* tipVector, unsigned char* tipX1,
                    unsigned char* tipX2, int n, double* left, double* right,
                    int* wgt, int* scalerIncrement, double* umpLeft,
                    double* umpRight)
{
...
#pragma vector aligned
        ...
        t2 = _mm512_castsi512_pd(
          _mm512_and_epi64(_mm512_castpd_si512(t2), absMask_MIC));
...
double
evaluateGAMMA_MIC(int* wgt, double* x1_start, double* x2_start,
...
void
sumGAMMA_MIC(int tipCase, double* sumtable, double* x1_start, double* x2_start,
...
void
coreGTRGAMMA_MIC(const int upper, double* sumtable,
                 volatile double* ext_dlnLdlz, volatile double* ext_d2lnLdlz2,
                 double* EIGN, double* gammaRates, double lz, int* wgt)
{
...
#pragma unroll(8)
    for (int j = 0; j < 8; j++) {
      ...
      inv_2 =
        _mm512_add_pd(inv_2, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                               _mm512_castpd_si512(inv_2), _MM_PERM_BADC)));
      ...
      d1_2 = _mm512_add_pd(d1_2, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                                   _mm512_castpd_si512(d1_2), _MM_PERM_BADC)));
      ...
      d2_2 = _mm512_add_pd(d2_2, _mm512_castsi512_pd(_mm512_permute4f128_epi32(
                                   _mm512_castpd_si512(d2_2), _MM_PERM_BADC)));
