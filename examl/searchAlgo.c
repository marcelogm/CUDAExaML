...
boolean
smooth(tree* tr, nodeptr p)
{
  nodeptr q;

  if (!update(tr, p))
    return FALSE;                      /*  Adjust branch */
  if (!isTip(p->number, tr->mxtips)) { /*  Adjust descendants */
    q = p->next;
    while (q != p) {
      if (!smooth(tr, q->back))
        return FALSE;
...
boolean
localSmooth(tree* tr, nodeptr p, int maxtimes)
{
  nodeptr q;
  int i;
...
void
insertInfoList(nodeptr node, double likelihood)
{
  int i;
  int min = 0;
...
boolean
smoothRegion(tree* tr, nodeptr p, int region)
{
  nodeptr q;
  ...
  if (region > 0) {
    if (!isTip(p->number, tr->mxtips)) {
      q = p->next;
      while (q != p) {
        if (!smoothRegion(tr, q->back, --region))
          return FALSE;
        q = q->next;
      }
...
boolean
regionalSmooth(tree* tr, nodeptr p, int maxtimes, int region)
{
  nodeptr q;
  int i;
...
nodeptr
removeNodeBIG(tree* tr, nodeptr p, int numBranches)
{
...
nodeptr
removeNodeRestoreBIG(tree* tr, nodeptr p)
{
  nodeptr q, r;
...
boolean
insertBIG(tree* tr, nodeptr p, nodeptr q, int numBranches)
{
  nodeptr r, s;
  int i;
  ...
  if (Thorough) {
    double zqr[NUM_BRANCHES], zqs[NUM_BRANCHES], zrs[NUM_BRANCHES], lzqr, lzqs,
      lzrs, lzsum, lzq, lzr, lzs, lzmax;
...
boolean
insertRestoreBIG(tree* tr, nodeptr p, nodeptr q)
{
  nodeptr r, s;
...
static void
restoreTopologyOnly(tree* tr, bestlist* bt, bestlist* bestML)
{
  nodeptr p = tr->removeNode;
  nodeptr q = tr->insertNode;
  double qz[NUM_BRANCHES], pz[NUM_BRANCHES], p1z[NUM_BRANCHES],
    p2z[NUM_BRANCHES];
  nodeptr p1, p2, r, s;
...
boolean
testInsertBIG(tree* tr, nodeptr p, nodeptr q)
{
...
void
addTraverseBIG(tree* tr, nodeptr p, nodeptr q, int mintrav, int maxtrav)
{
  if (--mintrav <= 0) {
    if (!testInsertBIG(tr, p, q))
      return;
  }
...
int
rearrangeBIG(tree* tr, nodeptr p, int mintrav, int maxtrav)
{
  double p1z[NUM_BRANCHES], p2z[NUM_BRANCHES], q1z[NUM_BRANCHES],
    q2z[NUM_BRANCHES];
  ...
  if (!isTip(p->number, tr->mxtips) && doP) {
    ...
    if (!isTip(p1->number, tr->mxtips) || !isTip(p2->number, tr->mxtips)) {
      ...
      if (!removeNodeBIG(tr, p, tr->numBranches))
        return badRear;
  ...
  if (!isTip(q->number, tr->mxtips) && maxtrav > 0 && doQ) {
    ...
    if ((!isTip(q1->number, tr->mxtips) &&
      ...
      if (!removeNodeBIG(tr, q, tr->numBranches))
        return badRear;

      mintrav2 = mintrav > 2 ? mintrav : 2;

      if (/*! q1->tip*/ !isTip(q1->number, tr->mxtips)) {
        addTraverseBIG(tr, q, q1->next->back, mintrav2, maxtrav);
        addTraverseBIG(tr, q, q1->next->next->back, mintrav2, maxtrav);
      }

      if (/*! q2->tip*/ !isTip(q2->number, tr->mxtips)) {
        addTraverseBIG(tr, q, q2->next->back, mintrav2, maxtrav);
        addTraverseBIG(tr, q, q2->next->next->back, mintrav2, maxtrav);
      }
...
double
treeOptimizeRapid(tree* tr, int mintrav, int maxtrav, analdef* adef,
                  bestlist* bt, bestlist* bestML)
{
...
boolean
testInsertRestoreBIG(tree* tr, nodeptr p, nodeptr q)
{
  if (Thorough) {
    if (!insertBIG(tr, p, q, tr->numBranches))
      return FALSE;
...
static void
writeTree(tree* tr, FILE* f)
{
  int x = tr->mxtips + 3 * (tr->mxtips - 1);
 ...
 */
static void
gatherDistributedCatInfos(tree* tr, int** rateCategory_result,
                          double** patrat_result)
{
  /*
 ...
 */
  ...
  int *countPerProc = (int *)NULL, *displPerProc = (int *)NULL;

  calculateLengthAndDisplPerProcess(tr, &countPerProc, &displPerProc);

  if (processID == 0) {
    *rateCategory_result =
      (int*)calloc((size_t)tr->originalCrunchedLength, sizeof(int));
    *patrat_result =
      (double*)calloc((size_t)tr->originalCrunchedLength, sizeof(double));
  }

  gatherDistributedArray(tr, (void**)patrat_result, tr->patrat_basePtr,
                         MPI_DOUBLE, countPerProc, displPerProc);
  gatherDistributedArray(tr, (void**)rateCategory_result,
                         tr->rateCategory_basePtr, MPI_INT, countPerProc,
                         displPerProc);
 ...
 */
static void
writeCheckpointInner(tree* tr, int* rateCategory, double* patrat, analdef* adef)
{
  int model;

  char extendedName[2048], buf[64];
  ...
  for (model = 0; model < tr->NumberOfModels; model++) {
    int dataType = tr->partitionData[model].dataType;

    myBinFwrite(&(tr->partitionData[model].numberOfCategories), sizeof(int), 1,
                f);
    myBinFwrite(tr->partitionData[model].perSiteRates, sizeof(double),
                tr->maxCategories, f);
    myBinFwrite(tr->partitionData[model].EIGN, sizeof(double),
                pLengths[dataType].eignLength, f);
    myBinFwrite(tr->partitionData[model].EV, sizeof(double),
                pLengths[dataType].evLength, f);
    myBinFwrite(tr->partitionData[model].EI, sizeof(double),
                pLengths[dataType].eiLength, f);

    myBinFwrite(tr->partitionData[model].freqExponents, sizeof(double),
                pLengths[dataType].frequenciesLength, f);
    myBinFwrite(tr->partitionData[model].frequencies, sizeof(double),
                pLengths[dataType].frequenciesLength, f);
    myBinFwrite(tr->partitionData[model].tipVector, sizeof(double),
                pLengths[dataType].tipVectorLength, f);
    myBinFwrite(tr->partitionData[model].substRates, sizeof(double),
                pLengths[dataType].substRatesLength, f);
    ...
    // myBinFwrite(tr->partitionData[model].weightsBuffer , sizeof(double), 4,
    // f);
    // myBinFwrite(tr->partitionData[model].weightExponentsBuffer ,
    // sizeof(double), 4, f);
    ...
    if (tr->partitionData[model].protModels == LG4M ||
      ...
      for (k = 0; k < 4; k++) {
        myBinFwrite(tr->partitionData[model].rawEIGN_LG4[k], sizeof(double),
                    pLengths[dataType].eignLength, f);
        myBinFwrite(tr->partitionData[model].EIGN_LG4[k], sizeof(double),
                    pLengths[dataType].eignLength, f);
        myBinFwrite(tr->partitionData[model].EV_LG4[k], sizeof(double),
                    pLengths[dataType].evLength, f);
        myBinFwrite(tr->partitionData[model].EI_LG4[k], sizeof(double),
                    pLengths[dataType].eiLength, f);
        myBinFwrite(tr->partitionData[model].frequencies_LG4[k], sizeof(double),
                    pLengths[dataType].frequenciesLength, f);
        myBinFwrite(tr->partitionData[model].tipVector_LG4[k], sizeof(double),
                    pLengths[dataType].tipVectorLength, f);
        myBinFwrite(tr->partitionData[model].substRates_LG4[k], sizeof(double),
                    pLengths[dataType].substRatesLength, f);
      }
    }
  ...
  if (ckp.state == MOD_OPT) {
    ...
    myBinFwrite(tr->treeStrings, sizeof(char),
                (size_t)tr->treeStringLength * (size_t)tr->numberOfTrees, f);
  }
...
void
writeCheckpoint(tree* tr, analdef* adef)
{
  int* rateCategory = (int*)NULL;

  double* patrat = (double*)NULL;
...
static void
readTree(tree* tr, FILE* f)
{
  int nodeNumber, x = tr->mxtips + 3 * (tr->mxtips - 1);

  nodeptr startAddress;
...

  {
    ...
    for (i = 0; i < x; i++) {
      if (addIt) {
        tr->nodeBaseAddress[i].next =
          (nodeptr)((size_t)tr->nodeBaseAddress[i].next + offset);
        tr->nodeBaseAddress[i].back =
          (nodeptr)((size_t)tr->nodeBaseAddress[i].back + offset);
      } else {
        tr->nodeBaseAddress[i].next =
          (nodeptr)((size_t)tr->nodeBaseAddress[i].next - offset);
        tr->nodeBaseAddress[i].back =
...
genericError(void)
{
  printBothOpen("\nError: command lines used in initial run and re-start from "
                "checkpoint do not match!\n");
}

static void
checkCommandLineArguments(tree* tr, analdef* adef)
{
  boolean match = TRUE;
...
static void
readCheckpoint(tree* tr, analdef* adef)
{
  int model;

  FILE* f = myfopen(binaryCheckpointInputName, "rb");
  ...
  if (tr->searchConvergenceCriterion && processID == 0) {
    ...
    if ((ckp.state == FAST_SPRS && ckp.fastIterations > 0) ||
      ...
      bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back, tr->mxtips,
                              tr->vLength, tr->h, 0, BIPARTITIONS_RF,
                              (branchInfo*)NULL, &bCounter, 1, FALSE, FALSE);
    ...
    if ((ckp.state == FAST_SPRS && ckp.fastIterations > 1) ||
      ...
      bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back, tr->mxtips,
                              tr->vLength, tr->h, 1, BIPARTITIONS_RF,
                              (branchInfo*)NULL, &bCounter, 1, FALSE, FALSE);
  ...
  if (tr->rateHetModel == CAT) {
    ...
    /* Andre coould you maybe add a drawing (scanned drawn by hand if you like)
     * documenting this layout ? => Andre: TODO  */

    while (aIter != aEnd) {
      if (aIter->procId == processID) {
        pInfo* partition = &(tr->partitionData[aIter->partitionId]);
        exa_off_t theOffset =
          pPos + (partition->lower + aIter->offset) * sizeof(double);
  ...
  for (model = 0; model < tr->NumberOfModels; model++) {
    ...
    myBinFread(&(tr->partitionData[model].numberOfCategories), sizeof(int), 1,
               f);
    myBinFread(tr->partitionData[model].perSiteRates, sizeof(double),
               tr->maxCategories, f);
    myBinFread(tr->partitionData[model].EIGN, sizeof(double),
               pLengths[dataType].eignLength, f);
    myBinFread(tr->partitionData[model].EV, sizeof(double),
               pLengths[dataType].evLength, f);
    myBinFread(tr->partitionData[model].EI, sizeof(double),
               pLengths[dataType].eiLength, f);

    myBinFread(tr->partitionData[model].freqExponents, sizeof(double),
               pLengths[dataType].frequenciesLength, f);
    myBinFread(tr->partitionData[model].frequencies, sizeof(double),
               pLengths[dataType].frequenciesLength, f);
    myBinFread(tr->partitionData[model].tipVector, sizeof(double),
               pLengths[dataType].tipVectorLength, f);
    myBinFread(tr->partitionData[model].substRates, sizeof(double),
               pLengths[dataType].substRatesLength, f);
    ...
    // myBinFread(tr->partitionData[model].weightsBuffer , sizeof(double), 4,
    // f);
    // myBinFread(tr->partitionData[model].weightExponentsBuffer ,
    // sizeof(double), 4, f);

    // LG4X end

    if (tr->partitionData[model].protModels == LG4X ||
        tr->partitionData[model].protModels == LG4M) {
      int k;

      for (k = 0; k < 4; k++) {
        myBinFread(tr->partitionData[model].rawEIGN_LG4[k], sizeof(double),
                   pLengths[dataType].eignLength, f);
        myBinFread(tr->partitionData[model].EIGN_LG4[k], sizeof(double),
                   pLengths[dataType].eignLength, f);
        myBinFread(tr->partitionData[model].EV_LG4[k], sizeof(double),
                   pLengths[dataType].evLength, f);
        myBinFread(tr->partitionData[model].EI_LG4[k], sizeof(double),
                   pLengths[dataType].eiLength, f);
        myBinFread(tr->partitionData[model].frequencies_LG4[k], sizeof(double),
                   pLengths[dataType].frequenciesLength, f);
        myBinFread(tr->partitionData[model].tipVector_LG4[k], sizeof(double),
                   pLengths[dataType].tipVectorLength, f);
        myBinFread(tr->partitionData[model].substRates_LG4[k], sizeof(double),
                   pLengths[dataType].substRatesLength, f);
      }
    }
    ...
    // only overwrite values of discrete gamma cats by calling makeGammaCats if
    // not using
    // LG4X!
    if (tr->rateHetModel != CAT &&
        !(tr->partitionData[model].protModels == LG4X))
      makeGammaCats(tr->partitionData[model].alpha,
                    tr->partitionData[model].gammaRates, 4, tr->useMedian);
  ...
  if (ckp.state == MOD_OPT) {
    ...
    myBinFread(tr->treeStrings, sizeof(char),
               (size_t)tr->treeStringLength * (size_t)tr->numberOfTrees, f);
  }
...
int
determineRearrangementSetting(tree* tr, analdef* adef, bestlist* bestT,
                              bestlist* bt, bestlist* bestML)
{
  const int MaxFast = 26;

  int i, maxtrav = 5, bestTrav = 5;

  double startLH = tr->likelihood;

  boolean impr = TRUE, cutoff = tr->doCutoff;
...
void
computeBIGRAPID(tree* tr, analdef* adef, boolean estimateModel)
{
  int i, impr, bestTrav = 0, treeVectorLength = 0, rearrangementsMax = 0,
               rearrangementsMin = 0, thoroughIterations = 0,
               fastIterations = 0;

  double lh = unlikely, previousLh = unlikely, difference, epsilon;

  bestlist *bestML, *bestT, *bt;
  ...
  /* initialize an additional data structure used by the search algo, all of
   this is pretty
  ...
  /* Thorough = 0 means that we will do fast SPR inbsertions without optimizing
   the
...
*/
  ...
  /* if we are not using a checkpoint and estimateModel is set to TRUE we call
   the function
   that optimizes model parameters, such as the CAT model assignment, the alpha
   paremeter
   or the rates in the GTR matrix. Otherwise we just optimize the branch
   lengths. Note that
   the second parameter of treeEvaluate() controls how many times we will
   iterate over all branches
   of the tree until we give up, provided that, the br-len opt. has not
   converged before.
*/
  ...
  /* save the current tree (which is the input tree parsed via -t in the bestT
   * list */

  saveBestTree(bestT, tr, TRUE);

  /* if the rearrangmenet radius has been set by the user ie. adef->initailSet
   == TRUE
   then just set the apppropriate parameter.
   Otherwise, call the function  determineRearrangementSetting() that seeks
   for the best radius by executing SPR moves on the initial tree with different
   radii
   and returns the smallest radius that yields the best log likelihood score
   after
...
*/
  ...
  if (!adef->initialSet) {
    if ((!adef->useCheckpoint) ||
        (adef->useCheckpoint && ckp.state == REARR_SETTING)) {
      bestTrav = adef->bestTrav =
        determineRearrangementSetting(tr, adef, bestT, bt, bestML);
  ...
  if (!(adef->useCheckpoint &&
        (ckp.state == FAST_SPRS || ckp.state == SLOW_SPRS))) {
  ...
  /* save the current tree again, while the topology has not changed, the branch
   lengths have changed in the meantime, hence
  ...
  /* this is for the additional RAxML heuristics described imn this paper here:
   ...
   A. Stamatakis,  F. Blagojevic, C.D. Antonopoulos, D.S. Nikolopoulos:
...
*/
  ...
  START_FAST_SPRS:
    ...
    /* save states of algorithmic/heuristic variables for printing the next
     * checkpoint */
   ...
   */
    ...
    /* this is the aforementioned convergence criterion that requires computing
     the RF,
    ...
    if (tr->searchConvergenceCriterion && processID == 0) {
      ...
      char* buffer = (char*)calloc(tr->treeStringLength, sizeof(char));

      if (fastIterations > 1)
        cleanupHashTable(tr->h, (fastIterations % 2));

      bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back, tr->mxtips,
                              tr->vLength, tr->h, fastIterations % 2,
      ...
      Tree2String(buffer, tr, tr->start->back, FALSE, TRUE, FALSE, FALSE, FALSE,
      ...
      if (fastIterations > 0) {
        ...
        {
          printBothOpen("ML fast search converged at fast SPR cycle %d with "
                        "stopping criterion\n",
                        fastIterations);
          printBothOpen("Relative Robinson-Foulds (RF) distance between "
                        "respective best trees after one succseful SPR cycle: "
                        "%f%s\n",
                        rrf, "%");
          ...
          printBothOpen("ML search convergence criterion fast cycle %d->%d "
                        "Relative Robinson-Foulds %f\n",
    ...
    /* set impr to 0 since in the immediately following for loop we check if the
     SPR moves above have generated
     a better tree */

    impr = 0;

    /* loop over the 20 best trees generated by the fast SPR moves, and check if
     they improve the likelihood after all of their branch lengths
     have been optimized */
    ...
    for (i = 1; i <= bt->nvalid; i++) {
      ...
      difference =
        ((tr->likelihood > previousLh) ? tr->likelihood - previousLh
                                       : previousLh - tr->likelihood);

      /* if the likelihood has improved save the current tree as best tree and
       * continue */
      /* note that we always compre this tree to the likelihood of the previous
       * best tree */
  ...
  /* needed for this RF-based convergence criterion that I actually describe in
   here:

   A. Stamatakis: "Phylogenetic Search Algorithms for Maximum Likelihood". In M.
   Elloumi, A.Y. Zomaya, editors.
   Algorithms in Computational Biology: techniques, Approaches and Applications,
   John Wiley and Sons
...
cleanup_fast:
  ...
  now we have jumped out of the loop that executes
   fast SPRs, and next we will execute a loop that executes throough SPR cycles
  (with SPR moves
   that optimize via newton-Raphson all adjacent branches to the insertion
  point)
   until no through SPR move can be found that improves the likelihood further.
  A classic
   hill climbing algo.
*/
  ...
  /* restore the currently best tree. this si actually required, because we do
   not know which tree
  ...
  /* optimize model params (including branch lengths) or just
   optimize branch lengths and leave the other model parameters (GTR rates,
   alhpa)
   alone */
  ...
  while (1) {
  /* once again if we want to restart from a checkpoint that was written during
     this loop we need
  ...
  START_SLOW_SPRS:
    if (adef->useCheckpoint && ckp.state == SLOW_SPRS) {
      ...
      /* otherwise we restore the currently best tree and load it from bestT
     into our tree data
     structuire tr */
      recallBestTree(bestT, 1, tr);
    ...
    if (impr) {
      /* if the logl has improved write out some stuff and adapt the
       * rearrangement radii */
      printResult(tr, adef, FALSE);
      /* minimum rearrangement radius */
      rearrangementsMin = 1;
      /* max radius, this is probably something I need to explain at the
       * whiteboard */
      ...
      if (tr->searchConvergenceCriterion && processID == 0) {
        ...
        bitVectorInitravSpecial(tr->bitVectors, tr->nodep[1]->back, tr->mxtips,
                                tr->vLength, tr->h, thoroughIterations % 2,
                                BIPARTITIONS_RF, (branchInfo*)NULL, &bCounter,
                                1, FALSE, FALSE);
        ...
        Tree2String(buffer, tr, tr->start->back, FALSE, TRUE, FALSE, FALSE,
        ...
        if (thoroughIterations > 0) {
          ...
          if (rrf <= 0.01) /* 1% cutoff */
          {
            printBothOpen("ML search converged at thorough SPR cycle %d with "
                          "stopping criterion\n",
                          thoroughIterations);
            printBothOpen("Relative Robinson-Foulds (RF) distance between "
                          "respective best trees after one succseful SPR "
                          "cycle: %f%s\n",
                          rrf, "%");
            goto cleanup;
          } else
            printBothOpen("ML search convergence criterion thorough cycle "
                          "%d->%d Relative Robinson-Foulds %f\n",
      ...
      /* if the lnl has not imrpved by the current SPR cycle adapt the min and
       * max rearrangemnt radii and try again */

      rearrangementsMax += adef->stepwidth;
      rearrangementsMin += adef->stepwidth;

      /* if we have already tried them then abandon this loop, the search has
       * converged */
    ...
    /* do a cycle of thorough SPR moves with the minimum and maximum
     * rearrangement radii */
    ...
    /* once again get the best 20 trees produced by the SPR cycle, load them
     from the bt tree list into tr
     optimize their branch lengths and figure out if the LnL of the tree has
     improved */
    ...
    for (i = 1; i <= bestML->nvalid; i++) {
      ...
      if (processID == 0) {
        ...
        Tree2String(tr->tree_string, tr, tr->start->back, TRUE, TRUE, FALSE,
                    FALSE, TRUE, SUMMARIZE_LH, FALSE, FALSE);
